    ...
    <div className="container-page grid gap-8 py-10 lg:grid-cols-[1fr_360px]">
      <section className="grid gap-4">
          ...
          >
            <div className="relative aspect-square overflow-hidden rounded-md bg-[#ede7db]">
              <Image
                ...
                alt={getClientProductImageAlt(item.product)}
                fill
                sizes="128px"
                ...
                <div>
                  ...
                  {isClientComingSoonProduct(item.product) ? (
                    ...
                    <p className="mt-2 text-xs font-bold text-muted">
                      {getClientPublicDeliveryEstimate(item.product)}
                    </p>
                  )}
                </div>
        ...
        {blockedItems.length > 0 ? (
          <div className="rounded-lg border border-[#fed7aa] bg-[#fff7ed] p-4 text-sm font-semibold leading-6 text-[#9a3412]">
            Certains produits du panier sont en validation et ne peuvent pas
            encore etre payes. Retirez-les pour continuer.
          </div>
        ) : null}
        <ShippingSelector
          selection={selection}
      ...
      <aside className="h-fit rounded-lg border border-line bg-paper p-5 shadow-sm">
        ...
        */}
        ...
        {validation.ok && blockedItems.length === 0 ? (
          <button
            type="button"
