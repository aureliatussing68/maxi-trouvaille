...
type DropshippingOrderOperationsItem = {
  priority: number;
  lane: string;
  orderNumber: string;
...
type DropshippingOrderOperationsBoard = {
  ...
  itemCount: number;
  items: DropshippingOrderOperationsItem[];
  sharedOperationsSource?: string;
  safety?: Record<string, boolean>;
};

type DropshippingOrderOperationsReadResult = {
  board: DropshippingOrderOperationsBoard;
  boardPath: string;
};
...
type IntegrationSourcingSessionField = {
  order: number;
  zone: string;
  key: string;
  label: string;
...
type IntegrationSourcingSessionImageTask = {
  order: number;
  fileName: string;
  role: string;
  depositPath: string;
...
type IntegrationSourcingSessionProduct = {
  priority: number;
  id: string;
  slug: string;
  name: string;
  ...
  imageDepositDir: string;
  searchHints: string[];
  fields: IntegrationSourcingSessionField[];
  imageTasks: IntegrationSourcingSessionImageTask[];
...
type IntegrationSourcingSession = {
  generatedAt: string;
  mode: string;
  status: string;
  packetCount: number;
  ...
  expectedImageCount: number;
  sources?: Record<string, string | null>;
  products: IntegrationSourcingSessionProduct[];
  safety?: Record<string, boolean>;
};

type IntegrationSourcingSessionReadResult = {
  session: IntegrationSourcingSession;
  sessionPath: string;
};

type IntegrationSourcingNextProof = {
  rank: number;
...
type IntegrationSourcingSessionAuditIssue = {
  scope: string;
  code: string;
  message: string;
};

type IntegrationSourcingSessionAuditRow = {
  id: string;
  slug: string;
  name: string;
  categoryId: string;
  ...
  productMarkdownPath: string;
  status: string;
  failures: IntegrationSourcingSessionAuditIssue[];
  warnings: IntegrationSourcingSessionAuditIssue[];
};

type IntegrationSourcingSessionAudit = {
  generatedAt: string;
  mode: string;
  status: string;
  productCount: number;
  packetCount: number;
  packetAlignmentOk: boolean;
  evidenceFieldCount: number;
  expectedImageCount: number;
  evidenceCsvRowCount: number;
  imageCsvRowCount: number;
  productFormJsonCount: number;
  productFormMarkdownCount: number;
  zoneCounts?: Record<string, number>;
  failureCount: number;
  warningCount: number;
  failures: IntegrationSourcingSessionAuditIssue[];
  warnings: IntegrationSourcingSessionAuditIssue[];
  rows: IntegrationSourcingSessionAuditRow[];
  safety?: Record<string, boolean>;
};

type IntegrationSourcingSessionAuditReadResult = {
  audit: IntegrationSourcingSessionAudit;
  auditPath: string;
};

type IntegrationNextProofAuditManualStates = {
  manualValue: string;
  evidenceNote: string;
...
type IntegrationNextProofAuditProof = {
  rank: number;
  productId: string;
  productName: string;
  categoryId: string;
  ...
  blockerCount: number;
  blockers: string[];
  manualStates: IntegrationNextProofAuditManualStates;
  adminHref: string;
};

type IntegrationNextProofAudit = {
  generatedAt: string;
  mode: string;
  status: string;
  proofCount: number;
  ...
  warningCount: number;
  sources?: Record<string, string | null>;
  structuralFailures?: IntegrationSourcingSessionAuditIssue[];
  warnings?: IntegrationSourcingSessionAuditIssue[];
  proofs: IntegrationNextProofAuditProof[];
  safety?: Record<string, boolean>;
};

type IntegrationNextProofAuditReadResult = {
  audit: IntegrationNextProofAudit;
  auditPath: string;
};
...
async function collectIntegrationSourcingSessionFiles(
  dir: string,
  out: string[] = [],
) {
  let entries: Dirent[];
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await collectIntegrationSourcingSessionFiles(fullPath, out);
    } else if (
      entry.isFile() &&
      entry.name.startsWith("SESSION_SOURCING_INTEGRATION_") &&
      entry.name.endsWith(".json")
    ) {
      out.push(fullPath);
    }
...
async function collectIntegrationSourcingSessionAuditFiles(
  dir: string,
  out: string[] = [],
) {
  let entries: Dirent[];
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await collectIntegrationSourcingSessionAuditFiles(fullPath, out);
    } else if (
      entry.isFile() &&
      entry.name.startsWith("AUDIT_SESSION_SOURCING_INTEGRATION_") &&
      entry.name.endsWith(".json")
    ) {
      out.push(fullPath);
    }
...
async function collectIntegrationNextProofAuditFiles(
  dir: string,
  out: string[] = [],
) {
  let entries: Dirent[];
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await collectIntegrationNextProofAuditFiles(fullPath, out);
    } else if (
      entry.isFile() &&
      entry.name.startsWith("AUDIT_PROCHAINES_PREUVES_SOURCING_INTEGRATION_") &&
      entry.name.endsWith(".json")
    ) {
      out.push(fullPath);
    }
...
async function collectDropshippingOrderOperationsFiles(
  dir: string,
  out: string[] = [],
) {
  let entries: Dirent[];
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await collectDropshippingOrderOperationsFiles(fullPath, out);
    } else if (
      entry.isFile() &&
      entry.name.startsWith("DROPSHIPPING_ORDER_OPERATIONS_BOARD_") &&
      entry.name.endsWith(".json")
    ) {
      out.push(fullPath);
    }
...
async function readLatestIntegrationSourcingSession(): Promise<IntegrationSourcingSessionReadResult | null> {
  const files = await collectIntegrationSourcingSessionFiles(
    path.join(boardRoot, "session-sourcing-integration-articles"),
  );
  const sorted = await Promise.all(
    files.map(async (filePath) => ({
      filePath,
  ...
  return {
    session: payload as IntegrationSourcingSession,
    sessionPath: path.relative(process.cwd(), latest.filePath),
  };
}

async function readLatestIntegrationSourcingSessionAudit(): Promise<IntegrationSourcingSessionAuditReadResult | null> {
  const files = await collectIntegrationSourcingSessionAuditFiles(
    path.join(boardRoot, "audit-session-sourcing-integration-articles"),
  );
  const sorted = await Promise.all(
    files.map(async (filePath) => ({
      filePath,
  ...
  return {
    audit: payload as IntegrationSourcingSessionAudit,
    auditPath: path.relative(process.cwd(), latest.filePath),
  };
}

async function readLatestIntegrationNextProofAudit(): Promise<IntegrationNextProofAuditReadResult | null> {
  const files = await collectIntegrationNextProofAuditFiles(
    path.join(boardRoot, "audit-prochaines-preuves-sourcing-integration-articles"),
  );
  const sorted = await Promise.all(
    files.map(async (filePath) => ({
      filePath,
  ...
  return {
    audit: payload as IntegrationNextProofAudit,
    auditPath: path.relative(process.cwd(), latest.filePath),
  };
}

async function readLatestDropshippingOrderOperations(): Promise<DropshippingOrderOperationsReadResult | null> {
  const files = await collectDropshippingOrderOperationsFiles(boardRoot);
  const sorted = await Promise.all(
    files.map(async (filePath) => ({
      filePath,
      mtimeMs: (await fs.stat(filePath)).mtimeMs,
  ...
  return {
    board: payload as DropshippingOrderOperationsBoard,
    boardPath: path.relative(process.cwd(), latest.filePath),
  };
}
...
function dropshippingOrderOperationsGeneratedAt(
  board: DropshippingOrderOperationsBoard,
) {
  const date = new Date(board.generatedAt ?? "");

  if (Number.isNaN(date.getTime())) {
    return board.generatedAtLocal || "Date inconnue";
  }
...
function dropshippingOrderLaneLabel(lane: string) {
  const labels: Record<string, string> = {
    WAIT_PAYMENT: "Paiement a confirmer",
    STOCK_EXCEPTION: "Stock a reprendre",
    READY_SUPPLIER_PREP: "Preparation partenaire",
    WAIT_TRACKING: "Suivi a ajouter",
    READY_FOLLOW_UP: "Suivi client",
    DONE: "Termine",
  };
...
function dropshippingOrderLaneClass(lane: string) {
  if (lane === "STOCK_EXCEPTION") {
    return "border-rose/25 bg-[#fff1f2] text-rose";
  }

  if (lane === "READY_SUPPLIER_PREP") {
    return "border-teal/25 bg-[#ecfdf5] text-teal";
  }

  if (lane === "WAIT_TRACKING" || lane === "READY_FOLLOW_UP") {
    return "border-brand-strong/30 bg-[#fff7d6] text-[#805000]";
  }
...
function dropshippingOrderOperationsRows(
  board: DropshippingOrderOperationsBoard,
) {
  return [...(board.items ?? [])].sort(
    (a, b) =>
      ...
      a.orderNumber.localeCompare(b.orderNumber, "fr"),
  );
}
...
function buildDropshippingOrderOperationsCsv(
  operations: DropshippingOrderOperationsReadResult,
) {
  const { board, boardPath } = operations;
  const headers = [
    "type_ligne",
    "priority",
    "lane",
    ...
    "stock_status",
    "order_status",
    "partner_prep_allowed",
    "next_action",
    "line_count",
    "sold_total",
    "estimated_margin",
    "missing_proofs",
    "customer_contact_ready",
    "tracking_ready",
    "partner_reference_ready",
    "updated_at",
    "source_path",
  ];
  const rows: Array<Array<boolean | number | string>> = [
    [
      "meta",
      0,
      board.mode,
      "Commandes dropshipping",
      `${board.counts.totalOrders} commandes`,
      `${board.counts.stockExceptions} stocks a reprendre`,
      `${board.counts.readySupplierPrep} preparations partenaire`,
      false,
      "Lecture seule, aucune commande fournisseur",
      board.itemCount,
      "",
      "",
      "",
  ...
  dropshippingOrderOperationsRows(board).forEach((row) => {
    rows.push([
      "commande",
      row.priority,
      dropshippingOrderLaneLabel(row.lane),
      row.orderNumber,
      row.paymentStatus,
      row.stockDecrementStatus,
      row.orderStatus,
      row.supplierActionAllowed,
      row.nextAction,
      row.lineCount,
      row.soldTotal,
      row.estimatedMargin,
      (row.missingProofs ?? []).join("|"),
      row.customerContactReady,
      row.hasTrackingNumber,
      row.hasSupplierOrderReference,
      row.updatedAt,
      boardPath,
    ]);
  });
...
function integrationSourcingSessionGeneratedAt(
  session: IntegrationSourcingSession,
) {
  const date = new Date(session.generatedAt);

  if (Number.isNaN(date.getTime())) {
    return session.generatedAt || "Date inconnue";
  }
...
function integrationSourcingSessionProducts(session: IntegrationSourcingSession) {
  return [...(session.products ?? [])].sort(
    (a, b) =>
      a.priority - b.priority ||
      b.priorityScore - a.priorityScore ||
      a.name.localeCompare(b.name, "fr"),
...
function integrationSourcingSessionAuditGeneratedAt(
  audit: IntegrationSourcingSessionAudit,
) {
  const date = new Date(audit.generatedAt);

  if (Number.isNaN(date.getTime())) {
    return audit.generatedAt || "Date inconnue";
  }
...
function integrationNextProofAuditGeneratedAt(audit: IntegrationNextProofAudit) {
  const date = new Date(audit.generatedAt);
...
function integrationSourcingSessionAuditRows(
  audit: IntegrationSourcingSessionAudit,
) {
  return [...(audit.rows ?? [])].sort(
    (a, b) =>
      a.priority - b.priority ||
      a.name.localeCompare(b.name, "fr"),
  );
...
function integrationSourcingZoneCounts(
  products: IntegrationSourcingSessionProduct[],
) {
  const counts = new Map<string, number>();
  ...
  for (const product of products) {
    for (const field of product.fields ?? []) {
      counts.set(field.zone, (counts.get(field.zone) ?? 0) + 1);
    }
  }
...
function isMissingIntegrationProofValue(value: string | boolean) {
...
function integrationSourcingNextProofs(
  products: IntegrationSourcingSessionProduct[],
  limit = 5,
): IntegrationSourcingNextProof[] {
  const rows: Omit<IntegrationSourcingNextProof, "rank">[] = [];
  ...
  for (const product of products) {
    ...
    for (const field of fields) {
      ...
      const isMissing = isMissingIntegrationProofValue(field.currentValue);
      const isHold = field.status.toUpperCase().includes("HOLD");
      ...
      rows.push({
        ...
        adminHref: field.adminHref || product.adminLinks?.proof || "/admin/preuves-partenaires?status=hold#top-verification",
...
function integrationNextProofAuditKey(
  proof: Pick<IntegrationNextProofAuditProof, "rank" | "productId" | "fieldKey">,
) {
  return `${proof.rank}:${proof.productId}:${proof.fieldKey}`;
}

function integrationNextProofKey(proof: IntegrationSourcingNextProof) {
  return `${proof.rank}:${proof.productId}:${proof.fieldKey}`;
}

function integrationNextProofAuditBlockers(
  audit: IntegrationNextProofAudit,
  limit = 8,
) {
  const counts = new Map<string, number>();
...
function buildIntegrationSourcingEvidenceCsv(
  sourcingSession: IntegrationSourcingSessionReadResult,
) {
  const headers = [
    "priority",
    "product_id",
  ...
  const rows = integrationSourcingSessionProducts(sourcingSession.session).flatMap(
    (product) =>
...
function buildIntegrationSourcingImagesCsv(
  sourcingSession: IntegrationSourcingSessionReadResult,
) {
  const headers = [
    "priority",
...
function buildIntegrationSourcingNextProofsCsv(
  proofs: IntegrationSourcingNextProof[],
) {
  const headers = [
    "rank",
    "product_priority",
...
function buildIntegrationNextProofAuditCsv(
  auditResult: IntegrationNextProofAuditReadResult,
) {
  const { audit, auditPath } = auditResult;
  const headers = [
    "rank",
    "product_id",
    "product_name",
  ...
  const rows = (audit.proofs ?? []).map((proof) => [
    proof.rank,
    proof.productId,
    proof.productName,
    proof.categoryId,
    ...
    proof.fieldKey,
    proof.fieldLabel,
    String(proof.rowPresent),
    proof.manualStates?.manualValue ?? "missing",
    proof.manualStates?.evidenceNote ?? "missing",
    proof.manualStates?.captureOrFilePath ?? "missing",
    proof.manualStates?.checkedSameArticle ?? "missing",
    proof.manualStates?.moussValidation ?? "missing",
    proof.manualStates?.finalDecision ?? "hold_or_missing",
...
function buildIntegrationSourcingSessionAuditCsv(
  auditResult: IntegrationSourcingSessionAuditReadResult,
) {
  const { audit, auditPath } = auditResult;
  const headers = [
    "priority",
  ...
  const rows = integrationSourcingSessionAuditRows(audit).map((row) => [
    row.priority,
    row.id,
    row.name,
    row.categoryId,
...
export default async function AdminPilotagePage() {
  ...
  const [
    ...
    integrationSourcingSession,
    integrationSourcingSessionAudit,
    integrationNextProofAudit,
    dropshippingOrderOperations,
  ] = await Promise.all([
    readLatestBoard(),
    ...
    readLatestIntegrationSourcingSession(),
    readLatestIntegrationSourcingSessionAudit(),
    readLatestIntegrationNextProofAudit(),
  ...
  const integrationExecutionExportHref = integrationExecution
    ...
    : null;
  const dropshippingOrderOperationsExportFilename =
    "maxi-commandes-dropshipping-operations.csv";
  const dropshippingOrderOperationsExportHref = dropshippingOrderOperations
    ? `data:text/csv;charset=utf-8,${encodeURIComponent(
        buildDropshippingOrderOperationsCsv(dropshippingOrderOperations),
      )}`
    : null;
  const integrationSourcingEvidenceExportFilename =
    "maxi-session-sourcing-integration-preuves.csv";
  const integrationSourcingEvidenceExportHref = integrationSourcingSession
    ? `data:text/csv;charset=utf-8,${encodeURIComponent(
        buildIntegrationSourcingEvidenceCsv(integrationSourcingSession),
      )}`
    : null;
  const integrationSourcingImagesExportFilename =
    "maxi-session-sourcing-integration-images.csv";
  const integrationSourcingImagesExportHref = integrationSourcingSession
    ? `data:text/csv;charset=utf-8,${encodeURIComponent(
        buildIntegrationSourcingImagesCsv(integrationSourcingSession),
      )}`
    : null;
  const integrationSourcingAuditExportFilename =
    "maxi-audit-session-sourcing-integration.csv";
  const integrationSourcingAuditExportHref = integrationSourcingSessionAudit
    ? `data:text/csv;charset=utf-8,${encodeURIComponent(
        buildIntegrationSourcingSessionAuditCsv(integrationSourcingSessionAudit),
      )}`
    : null;
  ...
  const dropshippingOrderRows = dropshippingOrderOperations
    ? dropshippingOrderOperationsRows(dropshippingOrderOperations.board)
    : [];
  const dropshippingOrderTopRows = dropshippingOrderRows.slice(0, 6);
  const dropshippingOrderLaneCounts = dropshippingOrderRows.reduce(
    (counts, row) => {
  ...
  const integrationProofZoneEntries = integrationExecution
    ...
    : [];
  const integrationSessionProducts = integrationSourcingSession
    ? integrationSourcingSessionProducts(integrationSourcingSession.session)
    : [];
  const integrationSessionZoneEntries = integrationSourcingSession
    ? integrationSourcingZoneCounts(integrationSessionProducts)
    : [];
  const integrationSessionAuditRows = integrationSourcingSessionAudit
    ? integrationSourcingSessionAuditRows(integrationSourcingSessionAudit.audit)
    : [];
  const integrationSessionAuditOk =
    integrationSourcingSessionAudit?.audit.status ===
      "OK_SESSION_SOURCING_HOLD_SYNC" &&
    (integrationSourcingSessionAudit.audit.failureCount ?? 0) === 0;
  const integrationNextProofs = integrationSourcingNextProofs(
    integrationSessionProducts,
  );
  const integrationNextProofsExportFilename =
    "maxi-prochaines-preuves-sourcing-integration.csv";
  const integrationNextProofsExportHref = integrationNextProofs.length
    ? `data:text/csv;charset=utf-8,${encodeURIComponent(
        buildIntegrationSourcingNextProofsCsv(integrationNextProofs),
      )}`
    : null;
  const integrationNextProofAuditByKey = new Map(
    (integrationNextProofAudit?.audit.proofs ?? []).map((proof) => [
      integrationNextProofAuditKey(proof),
      proof,
    ]),
  );
  const integrationNextProofAuditBlockerEntries = integrationNextProofAudit
    ? integrationNextProofAuditBlockers(integrationNextProofAudit.audit)
    : [];
  const integrationNextProofAuditExportFilename =
    "maxi-audit-prochaines-preuves-sourcing-integration.csv";
  const integrationNextProofAuditExportHref = integrationNextProofAudit
    ? `data:text/csv;charset=utf-8,${encodeURIComponent(
        buildIntegrationNextProofAuditCsv(integrationNextProofAudit),
      )}`
    : null;
      ...
      <section className="container-page grid grid-cols-1 gap-8 py-10">
        ...
        <section className="rounded-lg border border-line bg-paper p-5 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">
                File lecture seule issue du dernier board operations. Elle remonte
                les commandes payees dont le stock webhook doit etre repris, puis
                les preparations partenaire possibles apres validation humaine.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Link
              ...
              {dropshippingOrderOperationsExportHref ? (
                <a
                  href={dropshippingOrderOperationsExportHref}
                  download={dropshippingOrderOperationsExportFilename}
                  className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-line px-4 text-sm font-black hover:bg-[#f1eadf]"
                >
          ...
          {dropshippingOrderOperations ? (
            <>
              <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5">
                {[
                  {
                    label: "Commandes",
                    value: dropshippingOrderOperations.board.counts.totalOrders,
                    className: "bg-[#f8fafc] text-foreground",
                  },
                  {
                    label: "Payees",
                    value: dropshippingOrderOperations.board.counts.paidOrders,
                    className: "bg-[#ecfdf5] text-teal",
                  },
                  {
                    label: "Stock a reprendre",
                    value:
                      dropshippingOrderOperations.board.counts.stockExceptions,
                    className:
                      dropshippingOrderOperations.board.counts.stockExceptions > 0
                        ? "bg-[#fff1f2] text-rose"
                        : "bg-[#ecfdf5] text-teal",
                  },
                  {
                    label: "Pretes partenaire",
                    value:
                      dropshippingOrderOperations.board.counts.readySupplierPrep,
                    className: "bg-[#fff8e6] text-[#8a5a00]",
                  },
                  {
                    label: "Derniere synchro",
                    value: dropshippingOrderOperationsGeneratedAt(
                      dropshippingOrderOperations.board,
                    ),
                    className: "bg-[#faf7f0] text-foreground",
                  },
                ].map((metric) => (
                  <div
                    key={metric.label}
                    className={`rounded-md p-4 ${metric.className}`}
                  >
              ...
              <div
                className={`mt-4 rounded-md border p-4 ${
                  dropshippingOrderOperations.board.counts.stockExceptions > 0
                    ? "border-rose/25 bg-[#fff1f2] text-rose"
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="min-w-0">
                    ...
                    <h3 className="mt-2 text-lg font-black">
                      {dropshippingOrderOperations.board.counts.stockExceptions >
                      0
                        ? "Reprendre le stock avant toute preparation partenaire"
                        : "Aucune reprise stock locale detectee"}
                    </h3>
                    <p className="mt-2 text-sm font-bold leading-6">
                      Le board ne contient aucune adresse client ni aucune URL
                      fournisseur. Il sert a prioriser les reprises locales et la
                      preparation manuelle, sans paiement ni commande fournisseur
                      automatique.
                    </p>
                  </div>
                  <span className="inline-flex min-h-10 max-w-full items-center rounded-md border border-current bg-white/70 px-3 py-2 text-center text-xs font-black uppercase leading-5 break-all lg:shrink-0">
                    {dropshippingOrderOperations.board.mode}
                  </span>
                </div>
              </div>

              <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.6fr)]">
                <div className="rounded-md border border-line bg-[#faf7f0] p-4">
                  <div className="flex items-center gap-2">
                    <ClipboardList size={18} aria-hidden="true" />
                    <h3 className="text-lg font-black">Lanes operations</h3>
                  </div>
                  <div className="mt-4 space-y-3">
                    {[
                      ...
                      <div
                        key={lane}
                        className={`rounded-md border p-3 ${dropshippingOrderLaneClass(
                          lane,
                        )}`}
                      >
                        <div className="flex items-start justify-between gap-3">
                          <p className="text-sm font-black">
                            {dropshippingOrderLaneLabel(lane)}
                          </p>
                          <span className="rounded-full bg-white px-2 py-1 text-xs font-black text-foreground">
                            {dropshippingOrderLaneCounts.get(lane) ?? 0}
                          </span>
                        </div>
                      </div>
                    ))}
                  </div>
                  <p className="mt-4 break-all rounded-md bg-white p-3 text-xs font-bold text-muted">
                    Source: {dropshippingOrderOperations.boardPath}
                  </p>
                </div>
                ...
                <div className="min-w-0 rounded-md border border-line bg-white p-4">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <span className="rounded-full border border-line bg-[#faf7f0] px-3 py-2 text-xs font-black uppercase text-muted">
                      {dropshippingOrderOperations.board.itemCount} ligne(s)
                    </span>
                  </div>

                  {dropshippingOrderTopRows.length ? (
                    <div className="mt-4 divide-y divide-line">
                      {dropshippingOrderTopRows.map((row) => (
                        <article
                          key={`${row.orderNumber}-${row.priority}`}
                          className="grid grid-cols-1 gap-3 py-4 lg:grid-cols-[minmax(0,1fr)_auto]"
                        >
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <span
                                className={`rounded-full border px-2 py-1 text-xs font-black uppercase ${dropshippingOrderLaneClass(
                                  row.lane,
                                )}`}
                              >
                                {dropshippingOrderLaneLabel(row.lane)}
                              ...
                              <span className="rounded-full border border-line px-2 py-1 text-xs font-black text-muted">
                                stock {row.stockDecrementStatus}
                              </span>
                            </div>
                            ...
                            <p className="mt-2 text-xs font-bold text-muted">
                              {row.lineCount} ligne(s) | total {row.soldTotal} |
                              marge estimee {row.estimatedMargin}
                            </p>
                            {row.missingProofs.length ? (
                              <p className="mt-2 break-words rounded-md bg-[#fff8e6] p-2 text-xs font-bold text-[#8a5a00]">
                                Preuves manquantes: {row.missingProofs.join(", ")}
                              </p>
                            ) : null}
                          </div>
                          <div className="flex min-w-40 flex-col gap-2 lg:items-end">
                            <span
                              className={`rounded-full border px-3 py-2 text-xs font-black uppercase ${dropshippingOrderLaneClass(
                                row.supplierActionAllowed
                                  ? "READY_SUPPLIER_PREP"
                                  : row.lane,
                              )}`}
                    ...
                    <div className="mt-4 rounded-md border border-teal/25 bg-[#ecfdf5] p-4 text-sm font-black text-teal">
                      Aucune commande dropshipping locale a reprendre pour
                      le moment. Relancer `npm run catalog:order-operations-board`
                      apres les prochains paiements ou tests de commande.
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="mt-5 rounded-md border border-amber-300 bg-[#fff8e6] p-4 text-sm font-bold text-[#8a5a00]">
              Lancez `npm run catalog:order-operations-board` pour afficher la
              file operations commandes dans ce cockpit.
            </div>
          )}
        </section>
        ...
        <section className="rounded-lg border border-line bg-paper p-5 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <p className="text-sm font-black uppercase text-teal">
                Session sourcing terrain
              </p>
              <h2 className="mt-2 text-2xl font-black">
                Preuves a remplir avant revue Mouss
              </h2>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">
                Feuille de route terrain issue des packets prioritaires: champs de
                preuve, cout fournisseur max cible, depot WebP exact et liens admin
                internes par zone. Rien ne publie et rien ne contacte un fournisseur.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              {integrationSourcingEvidenceExportHref ? (
                <a
                  href={integrationSourcingEvidenceExportHref}
                  download={integrationSourcingEvidenceExportFilename}
                  className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-foreground px-4 text-sm font-black text-white hover:bg-[#2b2b2b]"
                >
                  Export preuves CSV
                  <Download size={16} aria-hidden="true" />
              ...
              {integrationSourcingImagesExportHref ? (
                <a
                  href={integrationSourcingImagesExportHref}
                  download={integrationSourcingImagesExportFilename}
                  className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-line px-4 text-sm font-black hover:bg-[#f1eadf]"
                >
                  Export images CSV
                  <Download size={16} aria-hidden="true" />
              ...
              {integrationSourcingAuditExportHref ? (
                <a
                  href={integrationSourcingAuditExportHref}
                  download={integrationSourcingAuditExportFilename}
                  className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-line px-4 text-sm font-black hover:bg-[#f1eadf]"
                >
                  Export audit CSV
                  <Download size={16} aria-hidden="true" />
              ...
              {integrationNextProofsExportHref ? (
                <a
                  href={integrationNextProofsExportHref}
                  download={integrationNextProofsExportFilename}
                  className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-line px-4 text-sm font-black hover:bg-[#f1eadf]"
                >
                  Export prochaines preuves
                  <Download size={16} aria-hidden="true" />
                </a>
              ) : null}
              {integrationNextProofAuditExportHref ? (
                <a
                  href={integrationNextProofAuditExportHref}
                  download={integrationNextProofAuditExportFilename}
                  className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-line px-4 text-sm font-black hover:bg-[#f1eadf]"
                >
                  Export audit preuves
                  <Download size={16} aria-hidden="true" />
                </a>
              ) : null}
              <Link
                href="/admin/preuves-sourcing"
                className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-line px-4 text-sm font-black hover:bg-[#f1eadf]"
              >
                Atelier preuves
                <ArrowRight size={16} aria-hidden="true" />
          ...
          {integrationSourcingSession ? (
            <>
              <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5">
                <div className="rounded-md bg-[#ecfdf5] p-4">
                  ...
                  <p className="mt-2 text-2xl font-black text-teal">
                    {integrationSourcingSession.session.productCount}
                  </p>
                </div>
                <div className="rounded-md bg-[#fff8e6] p-4">
                  <p className="text-xs font-black uppercase text-muted">
                  ...
                  <p className="mt-2 text-2xl font-black">
                    {integrationSourcingSession.session.evidenceFieldCount}
                  </p>
                </div>
                <div className="rounded-md bg-[#fff8e6] p-4">
                  <p className="text-xs font-black uppercase text-muted">
                  ...
                  <p className="mt-2 text-2xl font-black">
                    {integrationSourcingSession.session.expectedImageCount}
                  </p>
                </div>
                <div className="rounded-md bg-[#fff1f2] p-4">
                  <p className="text-xs font-black uppercase text-muted">
                  ...
                  <p className="mt-2 break-words text-sm font-black text-rose">
                    {integrationSourcingSession.session.status}
                  </p>
                </div>
                <div className="rounded-md bg-[#faf7f0] p-4">
                  <p className="text-xs font-black uppercase text-muted">
                  ...
                  <p className="mt-2 text-sm font-black">
                    {integrationSourcingSessionGeneratedAt(
                      integrationSourcingSession.session,
                    )}
                  </p>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs font-black uppercase">
                        Audit session terrain
                      </p>
                    </div>
                    <h3 className="mt-2 break-words text-lg font-black">
                      {integrationSourcingSessionAudit
                        ? compactStatus(integrationSourcingSessionAudit.audit.status)
                        : "Audit absent"}
                    </h3>
                    <p className="mt-2 text-sm font-bold leading-6">
                      {integrationSourcingSessionAudit
                        ? `${integrationSourcingSessionAudit.audit.failureCount} echec structurel, ${integrationSourcingSessionAudit.audit.warningCount} alerte, ${integrationSourcingSessionAudit.audit.productFormJsonCount} JSON et ${integrationSourcingSessionAudit.audit.productFormMarkdownCount} MD controles.`
                        : "Relancer npm run catalog:audit-integration-sourcing-session avant de remplir les preuves terrain."}
                    </p>
                    <p className="mt-2 break-all text-xs font-black uppercase">
                      {integrationSourcingSessionAudit
                        ? integrationSourcingSessionAudit.auditPath
                        : "Audit session non trouve"}
                    </p>
                  </div>
                  <span className="inline-flex min-h-10 shrink-0 items-center rounded-md border border-current bg-white/70 px-3 text-xs font-black uppercase">
                    {integrationSourcingSessionAudit
                      ? integrationSourcingSessionAuditGeneratedAt(
                          integrationSourcingSessionAudit.audit,
                        )
                      : "HOLD"}
                  </span>
                </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-5">
                    <div className="rounded-md bg-white/75 p-3">
                      ...
                      <p className="mt-1 text-2xl font-black">
                        {integrationSourcingSessionAudit.audit.packetAlignmentOk
                          ? "OK"
                          : "KO"}
                      </p>
                    </div>
                    <div className="rounded-md bg-white/75 p-3">
                      ...
                      <p className="mt-1 text-2xl font-black">
                        {integrationSourcingSessionAudit.audit.evidenceCsvRowCount}/
                        {integrationSourcingSessionAudit.audit.evidenceFieldCount}
                      </p>
                    </div>
                    <div className="rounded-md bg-white/75 p-3">
                      <p className="text-xs font-black uppercase text-muted">
                      ...
                      <p className="mt-1 text-2xl font-black">
                        {integrationSourcingSessionAudit.audit.imageCsvRowCount}/
                        {integrationSourcingSessionAudit.audit.expectedImageCount}
                      </p>
                    </div>
                    <div className="rounded-md bg-white/75 p-3">
                      <p className="text-xs font-black uppercase text-muted">
                      ...
                      <p className="mt-1 text-2xl font-black">
                        {integrationSourcingSessionAudit.audit.productFormJsonCount}/
                        {integrationSourcingSessionAudit.audit.productFormMarkdownCount}
                      </p>
                    </div>
                    <div className="rounded-md bg-white/75 p-3">
                      <p className="text-xs font-black uppercase text-muted">
                      ...
                      <p className="mt-1 text-2xl font-black">
                        {integrationSessionAuditRows.length}
                      </p>
                    </div>
                  </div>
                ) : null}
              ...
              {integrationNextProofs.length ? (
                <div className="mt-4 rounded-md border border-[#f2b84b] bg-[#fff8e6] p-4 text-[#6f4700]">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <p className="text-xs font-black uppercase">
                        Prochaines preuves a remplir
                      </p>
                      <h3 className="mt-2 text-lg font-black">
                        Top {integrationNextProofs.length} champs terrain
                      </h3>
                      <p className="mt-2 text-sm font-bold leading-6">
                        Commencer par ces champs dans cet ordre, puis relancer
                        audit session, audit preuves et audit intake avant toute
                        revue humaine.
                      </p>
                    </div>
                    <span className="inline-flex min-h-10 shrink-0 items-center rounded-md border border-current bg-white/70 px-3 text-xs font-black uppercase">
                      {integrationNextProofAudit
                        ? compactStatus(integrationNextProofAudit.audit.status)
                        : "Audit a lancer"}
                    </span>
                  </div>
                    ...
                    <>
                      <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-5">
                        <div className="rounded-md bg-white/80 p-3">
                          ...
                          <p className="mt-1 text-2xl font-black">
                            {integrationNextProofAudit.audit.proofCount}
                          </p>
                        </div>
                        <div className="rounded-md bg-white/80 p-3">
                          <p className="text-xs font-black uppercase text-muted">
                          ...
                          <p className="mt-1 text-2xl font-black text-teal">
                            {integrationNextProofAudit.audit.readyProofCount}
                          </p>
                        </div>
                        <div className="rounded-md bg-white/80 p-3">
                          <p className="text-xs font-black uppercase text-muted">
                          ...
                          <p className="mt-1 text-2xl font-black">
                            {integrationNextProofAudit.audit.holdProofCount}
                          </p>
                        </div>
                        <div className="rounded-md bg-white/80 p-3">
                          <p className="text-xs font-black uppercase text-muted">
                          ...
                          <p className="mt-1 text-2xl font-black">
                            {integrationNextProofAudit.audit.businessBlockerCount}
                          </p>
                        </div>
                        <div className="rounded-md bg-white/80 p-3">
                          <p className="text-xs font-black uppercase text-muted">
                          ...
                          <p className="mt-1 text-2xl font-black">
                            {integrationNextProofAudit.audit.structuralFailureCount}
                          </p>
                        </div>
                      </div>

                      {integrationNextProofAuditBlockerEntries.length ? (
                        <div className="mt-3 flex flex-wrap gap-2">
                          {integrationNextProofAuditBlockerEntries.map(
                            ([blocker, count]) => (
                              <span
                                key={blocker}
                                className="max-w-full break-all rounded-md border border-[#f6d38b] bg-white/80 px-3 py-2 text-xs font-black uppercase"
                              >
                                {blocker} x{count}
                              </span>
                            ),
                      ...
                      <p className="mt-3 break-all rounded-md bg-white/80 p-3 text-xs font-bold text-muted">
                        Audit: {integrationNextProofAudit.auditPath} |{" "}
                        {integrationNextProofAuditGeneratedAt(
                          integrationNextProofAudit.audit,
                        )}
                      </p>
                    </>
                  ) : (
                    <div className="mt-4 rounded-md border border-[#f6d38b] bg-white/80 p-3 text-sm font-black">
                      Relancer `npm run catalog:audit-integration-next-proofs-workpack`
                      pour voir les blocages du CSV terrain ici.
                    </div>
                  )}

                  <div className="mt-4 grid grid-cols-1 gap-3 xl:grid-cols-5">
                    {integrationNextProofs.map((proof) => {
                      const proofAudit = integrationNextProofAuditByKey.get(
                        integrationNextProofKey(proof),
                      );

                      return (
                        <article
                          key={`${proof.productId}-${proof.fieldKey}`}
                          className="flex min-h-[260px] flex-col justify-between rounded-md border border-[#f6d38b] bg-white p-3"
                        >
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="rounded-full bg-[#fff8e6] px-2 py-1 text-xs font-black">
                                #{proof.rank}
                              </span>
                              <span className="rounded-full border border-line px-2 py-1 text-xs font-black text-muted">
                                {proof.proofZone}
                              ...
                              {proofAudit ? (
                                <span
                                  className={`rounded-full border px-2 py-1 text-xs font-black uppercase ${statusClasses(
                                    proofAudit.status,
                                  )}`}
                                >
                                  {proofAudit.blockerCount} bloc.
                                </span>
                              ) : null}
                            </div>
                            <h4 className="mt-3 break-words text-sm font-black text-foreground">
                              {proof.fieldLabel}
                            </h4>
                            <p className="mt-2 break-words text-xs font-black uppercase text-muted">
                              {proof.productName}
                            </p>
                            <p className="mt-2 line-clamp-3 text-xs font-bold leading-5 text-muted">
                              {proof.expectedFormat}
                            </p>
                            <p className="mt-2 line-clamp-2 text-xs font-bold leading-5 text-[#8a5a00]">
                              Rejet si: {proof.rejectIf}
                            </p>
                            {proofAudit ? (
                              <dl className="mt-3 grid grid-cols-2 gap-2 border-t border-line pt-3 text-[11px] leading-4">
                                <div>
                                  <dt className="font-black uppercase text-muted">
                                    Valeur
                                  </dt>
                                  <dd className="mt-1 break-words font-bold">
                                    {proofAudit.manualStates.manualValue}
                                  </dd>
                                </div>
                                <div>
                                  <dt className="font-black uppercase text-muted">
                                  ...
                                  <dd className="mt-1 break-words font-bold">
                                    {proofAudit.manualStates.moussValidation}
                                  </dd>
                                </div>
                                <div>
                                  <dt className="font-black uppercase text-muted">
                                  ...
                                  <dd className="mt-1 break-words font-bold">
                                    {proofAudit.manualStates.checkedSameArticle}
                                  </dd>
                                </div>
                                <div>
                                  <dt className="font-black uppercase text-muted">
                                  ...
                                  <dd className="mt-1 break-words font-bold">
                                    {proofAudit.manualStates.finalDecision}
                                  </dd>
                                </div>
                              </dl>
                            ) : null}
                          ...
                          <Link
                            href={proof.adminHref}
                            className="focus-ring mt-3 inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-line px-3 text-xs font-black text-foreground hover:bg-[#f1eadf]"
                          >
                            Remplir
                            <ArrowRight size={14} aria-hidden="true" />
                          </Link>
                ...
                <div className="mt-4 rounded-md border border-teal/25 bg-[#ecfdf5] p-4 text-sm font-black text-teal">
                  Aucun champ terrain manquant detecte dans la session courante.
                  Relancer les audits complets avant revue humaine.
                </div>
              )}
              ...
              <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.6fr)]">
                <div className="rounded-md border border-line bg-[#faf7f0] p-4">
                  <div className="flex items-center gap-2">
                    <ShieldCheck size={18} aria-hidden="true" />
                    <h3 className="text-lg font-black">Zones a remplir</h3>
                  </div>
                  <div className="mt-4 space-y-3">
                    {integrationSessionZoneEntries.map(([zone, count]) => (
                      <div
                        key={zone}
                        className="rounded-md border border-line bg-white p-3"
                      >
                        <div className="flex items-start justify-between gap-3">
                          <p className="text-sm font-black">{zone}</p>
                          <span className="rounded-full bg-[#ecfdf5] px-2 py-1 text-xs font-black text-teal">
                            {count}
                          </span>
                        </div>
                  ...
                  <p className="mt-4 break-all rounded-md bg-white p-3 text-xs font-bold text-muted">
                    Source: {integrationSourcingSession.sessionPath}
                  </p>
                </div>
                ...
                <div className="min-w-0 rounded-md border border-line bg-white p-4">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <span className="max-w-full break-all rounded-full border border-amber-300 bg-[#fff8e6] px-3 py-2 text-xs font-black text-[#8a5a00]">
                      {integrationSourcingSession.session.mode}
                    </span>
                  </div>

                  <div className="mt-4 divide-y divide-line">
                    {integrationSessionProducts.map((product) => (
                      <article
                        key={product.id}
                        className="grid grid-cols-1 gap-3 py-4 lg:grid-cols-[minmax(0,1fr)_auto]"
                      >
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="rounded-full bg-[#ecfdf5] px-2 py-1 text-xs font-black text-teal">
                              #{product.priority} score {product.priorityScore}
                            </span>
                            <span className="rounded-full border border-line px-2 py-1 text-xs font-black text-muted">
                              {product.categoryId}
                            </span>
                            <span className="rounded-full border border-line px-2 py-1 text-xs font-black text-muted">
                              Preuves {product.evidenceProgress}
                            </span>
                            <span className="rounded-full border border-line px-2 py-1 text-xs font-black text-muted">
                              Images {product.imageProgress}
                            </span>
                          </div>
                          <h4 className="mt-2 break-words text-base font-black">
                            {product.name}
                          ...
                          <p className="mt-1 text-sm font-bold text-muted">
                            Cout fournisseur max cible: {product.supplierMaxCost}
                          </p>
                          <div className="mt-3 flex flex-wrap gap-2">
                            {(product.fields ?? []).slice(0, 5).map((field) => (
                              <span
                                key={`${product.id}-${field.key}`}
                                className="rounded-full border border-line bg-[#faf7f0] px-2 py-1 text-xs font-black text-muted"
                              >
                                {field.label}
                              </span>
                            ))}
                          </div>
                          <p className="mt-3 break-all rounded-md bg-[#faf7f0] p-2 text-xs font-bold text-muted">
                            {product.imageDepositDir}
                          </p>
                        </div>
                        <div className="flex min-w-40 flex-col gap-2 lg:items-end">
            ...
            <div className="mt-5 rounded-md border border-amber-300 bg-[#fff8e6] p-4 text-sm font-bold text-[#8a5a00]">
              Lancez `npm run catalog:integration-sourcing-session` pour afficher
              les formulaires terrain dans ce cockpit.
            </div>
          )}
        </section>
