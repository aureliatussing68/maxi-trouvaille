...
const actionRules: SourcingAction[] = [
  {
    priority: 1,
    blocker: "exact_product_url_must_be_https",
    label: "Trouver l URL HTTPS exacte",
    detail: "Remplir uniquement l URL de la fiche article exacte, pas une recherche ni une variante floue.",
    guardrail: "HOLD tant que l URL ne pointe pas vers le meme article exact.",
  },
  {
    priority: 2,
    blocker: "partner_name_too_short",
    label: "Verifier le vendeur stable",
    detail: "Renseigner le nom vendeur ou partenaire tel qu il apparait sur la fiche source verifiee.",
    guardrail: "Aucun nom generique, aucune source douteuse.",
  },
  {
    priority: 3,
    blocker: "supplier_sku_not_stable",
    label: "Relever la reference stable",
    detail: "Renseigner un SKU, item id ou identifiant reutilisable pour retrouver le meme article.",
    guardrail: "HOLD si la reference ne permet pas de commander exactement la meme variante.",
  },
  {
    priority: 4,
    blocker: "exact_variant_too_vague",
    label: "Verrouiller la variante",
    detail: "Preciser couleur, taille, dimensions, lot et accessoires inclus pour matcher le produit vendu.",
    guardrail: "HOLD si la variante ne colle pas aux images et a la description Maxi.",
  },
  {
    priority: 5,
    blocker: "supplier_price_cents_invalid",
    label: "Verifier le prix reel",
    detail: "Saisir un prix fournisseur reel en centimes, observe sur la fiche, sans estimation.",
    guardrail: "Aucun prix invente, aucune marge ouverte sans preuve.",
  },
  {
    priority: 6,
    blocker: "manual_value_missing_or_placeholder",
    label: "Remplir la valeur prouvee",
    detail: "Remplacer le vide ou placeholder par la preuve exacte attendue dans le CSV terrain.",
    guardrail: "La valeur brute reste hors interface et doit etre auditée avant revue.",
  },
  {
    priority: 7,
    blocker: "evidence_note_missing_or_placeholder",
    label: "Ajouter la note de preuve",
    detail: "Expliquer pourquoi la preuve correspond au meme article exact et a la bonne variante.",
    guardrail: "HOLD si la note ne justifie pas l exactitude.",
  },
  {
    priority: 8,
    blocker: "capture_or_file_path_missing_or_placeholder",
    label: "Deposer une capture locale",
    detail: "Ajouter une capture ou un fichier local qui prouve la valeur sans lien client visible.",
    guardrail: "Pas de fichier approximatif, pas d image generique.",
  },
  {
    priority: 9,
    blocker: "same_article_not_confirmed",
    label: "Confirmer meme article",
    detail: "Cocher seulement si le fournisseur, la variante, le prix et les visuels designent le meme article.",
    guardrail: "Sans confirmation, la fiche reste non vendable.",
  },
  {
    priority: 10,
    blocker: "mouss_validation_missing",
    label: "Demander validation Mouss",
    detail: "Faire valider humainement apres preuves, captures et audit sans erreur structurelle.",
    guardrail: "Validation humaine obligatoire avant toute suite business.",
  },
  {
    priority: 11,
    blocker: "final_decision_not_ready_review",
    label: "Garder la decision HOLD",
    detail: "Passer en READY_REVIEW uniquement apres audit OK et validation humaine explicite.",
    guardrail: "Ne jamais utiliser READY_REVIEW comme raccourci.",
  },
];
...
function cardRank(card: ProofCard) {
  const rawRank =
    card.workpackProof?.rank ?? card.auditProof?.rank ?? card.csvRow?.rank ?? 999;
  const rank = Number(rawRank);

  return Number.isFinite(rank) ? rank : 999;
}

function actionForCard(card: ProofCard): SourcingAction {
  const blockers = card.auditProof?.blockers ?? [];
  const matchedRule = actionRules.find((rule) => blockers.includes(rule.blocker));
  ...
  if (!card.auditProof) {
    return {
      priority: 95,
      blocker: "audit_missing",
      label: "Relancer l audit preuves",
      detail: "La preuve n a pas encore d etat d audit lisible dans le dernier rapport.",
      guardrail: "Ne pas remplir au juger sans audit synchronise.",
    };
  }

  if (!card.auditProof.rowPresent) {
    return {
      priority: 96,
      blocker: "csv_row_missing",
      label: "Regenerer le CSV terrain",
      detail: "La ligne de preuve manque dans le CSV a remplir.",
      guardrail: "Ne pas improviser une ligne hors workpack.",
    };
  }

  return {
    priority: 99,
    blocker: "manual_review_hold",
    label: "Revue humaine HOLD",
    detail: "Relancer les audits puis verifier la coherence de la preuve avant toute decision.",
    guardrail: "HOLD maintenu jusqu au GO humain explicite.",
  };
}

function buildActionPlan(cards: ProofCard[]) {
  return [...cards]
    .map((card) => ({ card, action: actionForCard(card) }))
    .sort((a, b) => a.action.priority - b.action.priority || cardRank(a.card) - cardRank(b.card));
}

function buildActionPlanCsv(data: PageData, plan: ReturnType<typeof buildActionPlan>) {
  const headers = [
    "ordre",
    "priorite_action",
    "rank_preuve",
  ...
  const rows = plan.map(({ card, action }, index) => [
    index + 1,
    action.priority,
    cardRank(card),
    card.workpackProof?.productName ?? card.auditProof?.productName ?? card.csvRow?.product_name ?? "",
    card.workpackProof?.proofZone ?? card.auditProof?.proofZone ?? card.csvRow?.proof_zone ?? "",
    card.workpackProof?.fieldLabel ?? card.auditProof?.fieldLabel ?? card.csvRow?.field_label ?? "",
    card.workpackProof?.fieldKey ?? card.auditProof?.fieldKey ?? card.csvRow?.field_key ?? "",
    action.blocker,
    action.label,
    action.detail,
    action.guardrail,
    card.auditProof?.status ?? card.workpackProof?.status ?? card.csvRow?.status ?? "",
    card.auditProof?.adminHref ?? card.workpackProof?.adminHref ?? card.csvRow?.admin_href ?? "",
    data.auditPath ?? "",
  ]);
...
export default async function AdminSourcingProofsPage() {
  ...
  const actionPlan = buildActionPlan(cards);
  const auditExportHref = `data:text/csv;charset=utf-8,${encodeURIComponent(
    buildAuditCsv(data, cards),
  )}`;
  const actionPlanExportHref = `data:text/csv;charset=utf-8,${encodeURIComponent(
    buildActionPlanCsv(data, actionPlan),
  )}`;
      ...
      <main className="container-page space-y-8 py-8 sm:py-10">
        <section className="rounded-md border border-line bg-paper p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <a
                href={actionPlanExportHref}
                download="maxi-plan-action-preuves-sourcing.csv"
                className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-line px-4 text-sm font-black hover:bg-[#f1eadf]"
              >
                Export plan action
                <Download size={16} aria-hidden="true" />
        ...
        <section className="rounded-md border border-line bg-paper p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              ...
              <h2 className="mt-2 text-2xl font-black">
                {actionPlan.length} action{actionPlan.length > 1 ? "s" : ""} a
                traiter
              </h2>
              <p className="mt-2 max-w-3xl text-sm font-bold leading-6 text-muted">
                Ordre deduit des blocages d audit, sans afficher les valeurs
                fournisseur. Commencer par les rangs les plus bas, puis relancer
          ...
          <div className="mt-5 grid gap-3 lg:grid-cols-5">
            {actionPlan.map(({ card, action }, index) => {
              const productName =
                card.workpackProof?.productName ??
                card.auditProof?.productName ??
                card.csvRow?.product_name ??
                "Produit a verifier";
              const fieldLabel =
                card.workpackProof?.fieldLabel ??
                card.auditProof?.fieldLabel ??
                card.csvRow?.field_label ??
                "Champ a remplir";

              return (
                <article
                  key={`action-${cardRank(card)}-${action.blocker}`}
                  className="min-w-0 rounded-md border border-line bg-[#fbfaf7] p-3"
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="inline-flex min-h-8 items-center rounded-md border border-[#f6d38b] bg-[#fff8e6] px-2 text-xs font-black uppercase text-[#8a5a00]">
                      #{cardRank(card)}
                    </span>
                  </div>
        ...
        <section className="rounded-md border border-line bg-paper p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-5 grid gap-4">
            {cards.map(({ workpackProof, auditProof, csvRow }) => {
              ...
              const nextAction = actionForCard({ workpackProof, auditProof, csvRow });
                ...
                >
                  ...
                  <div className="grid gap-3 lg:grid-cols-3">
                    <div className="rounded-md bg-white p-3">
                      ...
                      <p className="mt-2 text-sm font-bold leading-6">
                        Vente: {workpackProof?.targetSalePrice ?? "a verifier"}
                        <br />
                        Marge: {workpackProof?.targetMargin ?? "a verifier"}
                      </p>
                    </div>
                    <div className="rounded-md bg-white p-3">
                      <p className="text-xs font-black uppercase text-muted">
                      ...
                      <p className="mt-2 text-sm font-bold leading-6">
                        {workpackProof?.supplierMaxCost ?? "a verifier apres preuve"}
                      </p>
                    </div>
                    <div className="rounded-md bg-white p-3">
                      <p className="text-xs font-black uppercase text-muted">
