...
4. Branche Integration articles
   ...
   - Commandes cles: `npm run catalog:prepare-draft-backlog`, `npm run catalog:integrate-article-candidates`, `npm run catalog:apply-article-candidates`, `npm run catalog:audit-integration-articles`, `npm run catalog:integration-sourcing-packets`, `npm run catalog:audit-integration-sourcing-packets`, `npm run catalog:integration-execution-board`, `npm run catalog:integration-sourcing-session`, `npm run catalog:audit-integration-sourcing-session`, `npm run catalog:integration-sourcing-priority-board`, `npm run catalog:audit-integration-sourcing-priority-board`, `npm run catalog:integration-top3-sourcing-sprint`, `npm run catalog:audit-integration-top3-sourcing-sprint`, `npm run catalog:integration-top3-parallel-proofs-workpack`, `npm run catalog:audit-integration-top3-parallel-proofs-workpack`, `npm run catalog:integration-top3-webp-workpack`, `npm run catalog:audit-integration-top3-webp-workpack`, `npm run catalog:audit-integration-top3-webp-depot-files`, `npm run catalog:integration-top3-webp-depot-session`, `npm run catalog:audit-integration-top3-webp-depot-session`, `npm run catalog:audit-integration-top3-business-gate`, `npm run catalog:integration-top3-unblock-plan`, `npm run catalog:audit-integration-top3-unblock-plan`, `npm run catalog:integration-next-wave-sourcing-plan`, `npm run catalog:audit-integration-next-wave-sourcing-plan`, `npm run catalog:integration-next-wave-session`, `npm run catalog:audit-integration-next-wave-session`, `npm run catalog:integration-next-wave-active-batch`, `npm run catalog:audit-integration-next-wave-active-batch`, `npm run catalog:audit-integration-next-wave-active-batch-business-gate`, `npm run catalog:integration-next-wave-active-batch-micro-packs`, `npm run catalog:audit-integration-next-wave-active-batch-micro-packs`, `npm run catalog:integration-next-wave-active-batch-proof-intake`, `npm run catalog:audit-integration-next-wave-active-batch-proof-intake`, `npm run catalog:integration-next-wave-active-batch-webp-deposit-intake`, `npm run catalog:audit-integration-next-wave-active-batch-webp-deposit-intake`, `npm run catalog:integration-next-wave-active-batch-webp-validation-contracts`, `npm run catalog:audit-integration-next-wave-active-batch-webp-validation-contracts`, `npm run catalog:integration-next-wave-active-batch-mouss-review-board`, `npm run catalog:audit-integration-next-wave-active-batch-mouss-review-board`, `npm run catalog:integration-next-wave-active-batch-field-entry-pack`, `npm run catalog:audit-integration-next-wave-active-batch-field-entry-pack`, `npm run catalog:integration-next-proofs-workpack`, `npm run catalog:audit-integration-next-proofs-workpack`, `npm run catalog:audit-generated-artifact-leaks`, `npm run catalog:import-evidence-drafts`, `npm run catalog:partner-action-board`, `npm run catalog:partner-validation-packets`, `npm run catalog:all-partner-validation-queue`, `npm run catalog:fast-go-shortlist`.
...
npm run catalog:integration-next-wave-active-batch
npm run catalog:audit-integration-next-wave-active-batch
npm run catalog:audit-integration-next-wave-active-batch-business-gate
npm run catalog:integration-next-wave-active-batch-micro-packs
npm run catalog:audit-integration-next-wave-active-batch-micro-packs
npm run catalog:integration-next-wave-active-batch-proof-intake
npm run catalog:audit-integration-next-wave-active-batch-proof-intake
npm run catalog:integration-next-wave-active-batch-webp-deposit-intake
npm run catalog:audit-integration-next-wave-active-batch-webp-deposit-intake
npm run catalog:integration-next-wave-active-batch-webp-validation-contracts
npm run catalog:audit-integration-next-wave-active-batch-webp-validation-contracts
npm run catalog:integration-next-wave-active-batch-mouss-review-board
npm run catalog:audit-integration-next-wave-active-batch-mouss-review-board
npm run catalog:integration-next-wave-active-batch-field-entry-pack
npm run catalog:audit-integration-next-wave-active-batch-field-entry-pack
npm run catalog:audit-integration-next-wave-active-batch-field-completion-gate
npm run catalog:integration-next-wave-pending-batches-runway
npm run catalog:audit-integration-next-wave-pending-batches-runway
npm run catalog:integration-next-wave-pending-batches-field-entry-pack
npm run catalog:audit-integration-next-wave-pending-batches-field-entry-pack
...
La commande `catalog:audit-integration-top3-sourcing-sprint` controle ce sprint court avant execution terrain: 3 produits exactement, compteurs preuves/WebP, noms WebP attendus, dossiers depot autorises, liens admin internes, garde-fous HOLD et absence d'URL marketplace ou valeur sensible dans les sorties JSON/Markdown/CSV.

La commande `catalog:integration-top3-parallel-proofs-workpack` prepare un pack terrain parallele pour les 3 produits du sprint: 5 preuves critiques par produit (URL exacte, vendeur, SKU, variante, prix fournisseur), CSV remplissable, fiches Markdown/JSON et rappel des WebP exacts attendus. Elle ne remplit aucune valeur fournisseur, ne contacte personne et garde tout en HOLD. La commande `catalog:audit-integration-top3-parallel-proofs-workpack` verifie ensuite le volume 3x5, les liens admin internes, le CSV remplissable, les garde-fous et l'absence de fuite sensible.

La commande `catalog:integration-top3-webp-workpack` prepare le pack terrain images exactes du top 3: 9 WebP attendus, chemins de depot, CSV remplissable, fiches Markdown/JSON et cases de validation manuelle. Elle ne telecharge aucune image, ne copie rien dans `public/uploads` et ne publie rien. La commande `catalog:audit-integration-top3-webp-workpack` controle le volume 3x3, les noms `.webp`, les dossiers de depot, les liens admin, le CSV remplissable, les garde-fous et l'absence de fuite sensible.

La commande `catalog:audit-integration-top3-webp-depot-files` controle ensuite les fichiers vraiment deposes pour ce top 3: presence du WebP attendu, signature `RIFF/WEBP`, chemin limite a `business-maxi-trouvailles/depots-images-exactes/integration-articles` et absence de fuite sensible. Elle ne telecharge rien, ne cree aucune image, ne copie rien dans `public/uploads` et ne debloque jamais seule: statut `HOLD_TOP3_WEBP_FILES_MISSING` tant que les 9 WebP exacts manquent, ou `READY_TOP3_WEBP_FILES_FOR_HUMAN_REVIEW_HOLD` seulement pour revue humaine Mouss.

La commande `catalog:integration-top3-webp-depot-session` prepare les dossiers de depot locaux et les fichiers consigne pour ces 9 WebP top 3, directement dans `business-maxi-trouvailles/depots-images-exactes/integration-articles`. Elle ne cree aucun fichier image et ne remplit aucune preuve; elle sert seulement de plan de depot manuel. La commande `catalog:audit-integration-top3-webp-depot-session` verifie ensuite 3 dossiers, 9 WebP attendus, 3 fichiers consigne, chemins autorises, garde-fous et absence de fuite sensible.

La commande `catalog:audit-integration-top3-business-gate` fusionne les 15 preuves critiques du pack parallele et les 9 WebP exacts du top 3. Elle produit un seul feu rouge business: `HOLD_TOP3_BUSINESS_GATE_BLOCKED` tant qu'une preuve fournisseur/SKU/prix/variante ou une image exacte manque, ou `READY_TOP3_BUSINESS_GATE_HUMAN_REVIEW_HOLD` uniquement quand tout est pret pour revue humaine Mouss. Elle n'exporte pas de valeur fournisseur brute, ne publie rien et ne commande rien.

La commande `catalog:integration-top3-unblock-plan` transforme ce feu rouge top 3 en ordre terrain auditable: 24 actions maximum aujourd'hui, dont 15 preuves critiques et 9 WebP exacts, regroupees par produit et priorisees avant revue Mouss. Elle n'ajoute aucune valeur fournisseur, ne cherche rien dehors, ne telecharge aucune image, ne publie rien et ne commande rien. La commande `catalog:audit-integration-top3-unblock-plan` controle ensuite le volume, les liens admin internes, les garde-fous HOLD et l'absence de fuite sensible dans les sorties.

La commande `catalog:integration-next-wave-sourcing-plan` prepare la vague suivante apres le top 3: 12 produits HOLD, 60 preuves internes et 36 WebP exacts attendus, sans modifier le catalogue et sans exposer de valeur fournisseur. Elle exclut les produits top 3 pour eviter les doublons. La commande `catalog:audit-integration-next-wave-sourcing-plan` controle le volume, les liens admin internes, les dossiers de depot, les fichiers `.webp`, les garde-fous HOLD et l'absence de fuite sensible.

La commande `catalog:integration-next-wave-session` transforme cette vague en session terrain par lots: 3 lots de 4 produits, 12 fiches produit, 60 preuves CSV remplissables, 36 lignes images WebP et 12 README de depot local dans `business-maxi-trouvailles/depots-images-exactes/integration-articles`. Elle ne cree aucune image, ne telecharge rien, ne modifie pas le catalogue et garde tout en HOLD. La commande `catalog:audit-integration-next-wave-session` controle les lots, les CSV, les fiches, les README de depot, les garde-fous lecture seule et l'absence de fuite sensible avant que le board quotidien s'en serve.

La commande `catalog:integration-next-wave-active-batch` extrait le lot actif de cette session, par defaut `lot-01`: 4 produits, 20 preuves internes, 12 WebP exacts et 32 actions terrain. Elle sert de sprint court pour avancer sans se disperser, ne remplit aucune valeur fournisseur, ne cree aucune image et garde tout en HOLD. La commande `catalog:audit-integration-next-wave-active-batch` controle le volume, les CSV, les liens admin internes, les garde-fous et l'absence de fuite sensible. La commande `catalog:audit-integration-next-wave-active-batch-business-gate` fusionne ensuite les CSV preuves et images du lot actif avec les fichiers WebP deposes: elle bloque le lot en `HOLD_NEXT_WAVE_ACTIVE_BATCH_BUSINESS_GATE_BLOCKED` tant qu'une preuve, une confirmation Mouss ou un WebP exact manque, ou ouvre seulement `READY_NEXT_WAVE_ACTIVE_BATCH_BUSINESS_GATE_HUMAN_REVIEW_HOLD` pour revue humaine quand tout est complet.

La commande `catalog:integration-next-wave-active-batch-micro-packs` transforme le lot actif en 4 fiches terrain produit, 20 preuves a remplir, 12 WebP a deposer et 32 actions ordonnees. Elle ne remplit aucune valeur fournisseur, ne cree aucune image, ne telecharge rien et ne modifie pas le catalogue. La commande `catalog:audit-integration-next-wave-active-batch-micro-packs` controle les fiches JSON/Markdown, le CSV actions, les liens admin internes, les chemins locaux de preuves/WebP, les garde-fous HOLD et l'absence de fuite sensible.

La commande `catalog:integration-next-wave-active-batch-proof-intake` materialise ensuite les 20 preuves du lot actif en fichiers Markdown locaux sous `business-maxi-trouvailles/preuves-internes/integration-articles`, avec placeholders Mouss et statut `TO_FILL_HOLD`. Elle est non destructive: un fichier preuve deja present est conserve. Elle ne remplit aucune valeur fournisseur, ne cree aucune image et ne modifie pas le catalogue. La commande `catalog:audit-integration-next-wave-active-batch-proof-intake` verifie les 20 fichiers preuves, les 4 READMEs, le CSV de suivi, les marqueurs HOLD, les garde-fous et l'absence de fuite sensible.

La commande `catalog:integration-next-wave-active-batch-webp-deposit-intake` materialise les 12 emplacements WebP exacts du lot actif en dossiers locaux sous `business-maxi-trouvailles/depots-images-exactes/integration-articles`, avec 4 READMEs depot et un CSV de suivi. Elle ne cree aucun fichier image, ne telecharge rien, ne copie rien en public et laisse les 12 WebP en `TO_DEPOSIT_HOLD` tant que les vrais fichiers exacts ne sont pas deposes. La commande `catalog:audit-integration-next-wave-active-batch-webp-deposit-intake` verifie les dossiers, READMEs, chemins `.webp`, signatures des fichiers presents, compteurs manquants et absence de fuite sensible.

La commande `catalog:integration-next-wave-active-batch-webp-validation-contracts` ajoute un contrat JSON local a cote de chaque WebP attendu du lot actif: 12 contrats HOLD qui listent les preuves obligatoires, les controles d'acceptation, les interdits de copie publique et la validation Mouss requise. Elle ne cree aucun WebP, ne remplit aucune valeur fournisseur, ne debloque aucune fiche et garde chaque decision en `BLOCKED_HOLD`. La commande `catalog:audit-integration-next-wave-active-batch-webp-validation-contracts` verifie les 12 contrats, leur statut bloque, les signatures WebP presentes, les compteurs, les garde-fous et l'absence de fuite sensible.

La commande `catalog:integration-next-wave-active-batch-mouss-review-board` consolide les preuves internes, contrats WebP et gate business du lot actif en board de revue Mouss: 4 produits, preuves restantes, WebP manquants, contrats bloques et prochaine action humaine. Elle reste en lecture seule, n'envoie aucun message et ne debloque aucune vente; les fiches restent `BLOCKED_MOUSS_REVIEW_HOLD` tant que preuves et images exactes manquent. La commande `catalog:audit-integration-next-wave-active-batch-mouss-review-board` verifie le volume, les compteurs de blocage, les garde-fous et l'absence de fuite sensible.

La commande `catalog:integration-next-wave-active-batch-field-entry-pack` transforme la revue Mouss du lot actif en pack de saisie terrain: 4 fiches produit, 20 lignes preuves et 12 lignes WebP exacts, avec chemins locaux et actions attendues. Elle ne remplit aucune valeur, ne cree aucune image, ne contacte personne et garde les 32 entrees en HOLD. La commande `catalog:audit-integration-next-wave-active-batch-field-entry-pack` controle le volume, les fiches, les statuts HOLD, les garde-fous et l'absence de fuite sensible.

La commande `catalog:audit-integration-next-wave-active-batch-field-completion-gate` relit ensuite les 32 entrees du pack terrain contre les vrais fichiers locaux: preuves internes remplies, confirmations Mouss, WebP exacts presents avec signature valide et contrats WebP encore bloques en HOLD. Elle n'exporte aucune valeur fournisseur, ne copie aucune image en public et renvoie `HOLD_NEXT_WAVE_ACTIVE_BATCH_FIELD_COMPLETION_BLOCKED` tant qu'une preuve ou un WebP manque, ou seulement `READY_NEXT_WAVE_ACTIVE_BATCH_FIELD_COMPLETION_HUMAN_REVIEW_HOLD` pour revue humaine.

La commande `catalog:integration-next-wave-pending-batches-runway` prepare la suite sans toucher au lot actif: elle met en runway les lots `lot-02` et `lot-03`, soit 8 produits, 40 preuves et 24 WebP exacts attendus, avec statuts HOLD et chemins locaux. Elle ne remplace pas le lot actif, n'ajoute aucune valeur fournisseur, ne cree aucune image et ne publie rien. La commande `catalog:audit-integration-next-wave-pending-batches-runway` controle le volume, les garde-fous et l'absence de fuite sensible.

La commande `catalog:integration-next-wave-pending-batches-field-entry-pack` transforme cette runway en pack de saisie terrain interne: 8 fiches produit et un CSV de 64 entrees HOLD (40 preuves + 24 WebP), sans valeur fournisseur inventee, sans image creee ou telechargee, sans ecriture catalogue et sans publication. Son audit verifie les compteurs, les chemins de saisie, les statuts HOLD et l'absence de marqueur sensible avant toute suite.
...
La commande `catalog:daily-execution-board` rassemble les actions du jour: cockpit produit prioritaire, preuves produits partenaires, depots images categories, photos produits sprint, board Mouss images publiques, formulaire preuves texte images, coherence pipeline images publiques, sprint top 3 sourcing integration, pack parallele top 3 preuves critiques, pack WebP exacts top 3, audit depots WebP top 3, session depot WebP top 3, gate business top 3, plan deblocage top 3, prochaine vague sourcing, session prochaine vague, lot actif prochaine vague, micro-packs lot actif, preuves internes lot actif, depots WebP lot actif, pilotage sourcing integration, sources catalogue publiques, surface visuelle publique et garde-fous checkout/publication/admin/surprises. Elle remonte aussi les compteurs WebP manquants, preuves texte a remplir, lignes de formulaire, coherence 12 fiches/72 lignes, sprint top 3 integration, audit top 3, audit pack parallele top 3, audit pack WebP top 3, audit depots WebP top 3, session depot WebP top 3, gate business top 3, plan deblocage top 3, prochaine vague sourcing, session prochaine vague, lot actif prochaine vague, micro-packs lot actif, intake preuves lot actif, intake depots WebP lot actif, audit du board pilotage sourcing integration, scan anti-fuite, surveillance des imports publics, audit visuel anti-image stock/CDN, gardes admin API/pages, webhook stock Stripe, securite admin commandes, board operations commandes et pilotage protege, tout en restant en lecture seule.

La commande `catalog:audit-daily-execution-board` controle ce tableau avant de s'y fier: mode lecture seule, garde-fous sensibles, absence d'URL externe ou marqueur marketplace, actions images qui rappellent l'interdiction de copie publique/publication/commande, coherence pipeline images publiques OK, sprint top 3 sourcing integration en HOLD, pack parallele top 3 preuves audite OK, pack WebP top 3 audite OK, audit depots WebP top 3 exploitable, session depot WebP top 3 auditee OK, gate business top 3 exploitable, plan deblocage top 3 audite OK, prochaine vague sourcing auditee OK, session prochaine vague auditee OK, lot actif prochaine vague audite OK, micro-packs lot actif audites OK, intake preuves lot actif audite OK, intake depots WebP lot actif audite OK, board pilotage sourcing integration garde OK, sources catalogue publiques filtrees OK, surface visuelle publique OK, SEO produits HOLD OK, gardes admin API/pages OK, webhook stock Stripe OK, operations commandes redigees OK, pilotage operations protege OK, compteurs surface publique/checkout/surprises et copie image publique non appliquee.
