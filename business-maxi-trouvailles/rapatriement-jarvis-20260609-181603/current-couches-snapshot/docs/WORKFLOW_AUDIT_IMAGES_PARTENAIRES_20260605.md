...
## Controle rapide global

```powershell
npm run catalog:audit-partners
```

Le controle global lance:

- l'audit des images partenaires;
- l'audit des gates de publication partenaires.

Le controle doit retourner:

- `ok: true`
- aucune anomalie dans les resultats `images_partenaire`;
- aucune anomalie dans les resultats `gates_publication`.
...
- le nombre de produits partenaires;
- la repartition par statut et categorie;
- la repartition par validation image;
- les fiches brouillon avec leurs raisons HOLD restantes.

## Controle images seul
...
## Controle gates publication seul

```powershell
npm run catalog:audit-partner-gates
```

Ce controle verifie qu'un produit partenaire en HOLD fournisseur/prix/delai ne passe pas en `published` par erreur.
