...
const categoryImagePrefix = "/uploads/category-images/";
const imageHoldMessage =
  "HOLD - photo fournisseur exacte a valider avant publication";

function hasGenericCategoryImage(product) {
  return (
    typeof product.image === "string" &&
    product.image.startsWith(categoryImagePrefix)
  );
}

function getImageValidationHold() {
  return {
    status: "hold",
    checkedAt: "2026-06-05",
    reason:
      "Image de categorie generique detectee. Photo fournisseur exacte requise avant publication.",
    nextAction:
      "Selectionner une annonce fournisseur precise, verifier que la photo correspond au produit vendu, puis remplacer image/images avant remise en published.",
  };
}
...
function withPartnerPricing(product) {
  ...
  const shouldHoldForImage = hasGenericCategoryImage(product);

  return {
    ...product,
    status: shouldHoldForImage ? "draft" : product.status,
    price: salePriceCents,
    ...(shouldHoldForImage ? { imageValidation: getImageValidationHold() } : {}),
    dropshipping: {
    ...
    internalSourcing: {
      ...
      validationStatus: shouldHoldForImage
        ? imageHoldMessage
        : "a verifier manuellement avant commande fournisseur",
    },
  };
}
...
async function main() {
  ...
  console.log(
    `Imported ${productsToImport.length} partner products with +40% pricing. Generic-image products stay in draft/HOLD.`,
  );
}
