...
function formatCents(cents) {
  return `${(Math.max(0, Number(cents) || 0) / 100).toFixed(2)} EUR`;
}
...
function boardItem(order, operations) {
  const readiness = operations.getDropshippingOrderOperation(order);
  const missingProofs = operations.getDropshippingOrderLineProofGaps(order);
  ...
  return {
    ...
    soldTotal: formatCents(order.soldTotalCents),
    estimatedMargin: formatCents(order.estimatedMarginCents),
    missingProofs,
...
const items = orders
  .map((order) => boardItem(order, operations))
