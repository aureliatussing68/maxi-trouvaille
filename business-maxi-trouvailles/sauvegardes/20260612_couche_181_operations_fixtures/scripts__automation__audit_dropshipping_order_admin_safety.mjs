...
const checks = [
  ...
  check(
    ...
    !/fetch\(|onUpdate\(|PATCH|POST|supplierOrderReference/.test(operationsSource),
    "La readiness reste une lecture locale sans appel reseau ni commande fournisseur.",
  ),
  check(
    ...
    operationsBoardSource.includes("operationsModulePath") &&
      ...
      operationsBoardSource.includes("getDropshippingOrderLineProofGaps") &&
  ...
  check(
    "client_supplier_names_stay_hidden",
    !source.includes("AliExpress"),
    "Le panneau admin ne reintroduit aucune mention AliExpress dans la surface.",
  ),
];
