...
import { ArrowUpRight, BadgePercent, PackageCheck, ShieldCheck, Sparkles, Truck } from "lucide-react";
import { CategoryGrid } from "@/components/CategoryGrid";
import { PageHeader } from "@/components/PageHeader";
import { ProductCard } from "@/components/ProductCard";
import { categories, isDropshippingProduct, isNewProduct, isPromotionProduct } from "@/lib/catalog";
...
export const metadata: Metadata = {
  title: "Dropshipping",
  description:
    "Produits dropshipping partenaires, promotions et nouveautes vendus par Maxi Trouvaille avec paiement securise et suivi colis.",
};

export default async function DropshippingPage() {
  const allProducts = await getAllProducts();
  const dropshippingProducts = allProducts.filter(isDropshippingProduct);
  const highlightedProducts = [
    ...dropshippingProducts.filter(isPromotionProduct),
    ...dropshippingProducts.filter(
  ...
  return (
    <>
      <PageHeader
        ...
        title="Dropshipping Maxi Trouvaille"
        description="Produits partenaires vendus sur Maxi Trouvaille : paiement securise, service client Maxi Trouvaille, suivi colis et expedition par partenaire logistique."
      />
      ...
      <section className="container-page py-10">
        ...
        <CategoryGrid items={dropshippingCategories} compact />
      </section>
