...
export function CheckoutView() {
  ...
  const blockedItems = detailedItems.filter(
    (item) => !isProductPurchasable(item.product),
  );
      ...
      <section className="grid gap-5">
        <div className="rounded-lg border border-line bg-paper p-5 shadow-sm">
          <div className="flex items-start gap-3 rounded-lg bg-[#eef8f6] p-4 text-sm leading-6 text-[#115e59]">
            ...
            <p>
              Le paiement passe par Stripe Checkout. En production, le mode live
              reste bloque tant que STRIPE_ENABLE_LIVE_PAYMENTS n&apos;est pas active.
            </p>
          </div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">
            ...
            manipuler les donnees bancaires dans le site. Les paiements live
            doivent etre actives explicitement dans l&apos;environnement.
          </p>
        </div>
        ...
        {blockedItems.length > 0 ? (
          <div className="mt-5 rounded-lg border border-[#fed7aa] bg-[#fff7ed] p-4 text-sm font-semibold leading-6 text-[#9a3412]">
            Un produit du panier est marqué À venir. Le paiement est bloqué
            jusqu&apos;à son ouverture officielle.
          </div>
        ) : null}
