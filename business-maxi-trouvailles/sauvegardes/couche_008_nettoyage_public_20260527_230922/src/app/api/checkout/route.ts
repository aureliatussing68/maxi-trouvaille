...
export async function POST(request: Request) {
  ...
  if (!secretKey) {
    return NextResponse.json(
      {
        error:
          "Configuration Stripe absente. Ajoutez STRIPE_SECRET_KEY dans .env.local.",
      },
      { status: 400 },
    );
  }
  ...
  if (!stripeMode) {
    return NextResponse.json(
      {
        error:
          "Cle Stripe absente, placeholder ou paiement live non active. Utilisez sk_test_ ou activez STRIPE_ENABLE_LIVE_PAYMENTS=true avec une vraie cle live.",
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: `Erreur Stripe test : ${message}` },
      { status: 400 },
    );
  }
