...
export async function getPublicProducts() {
  return (await getAllProducts()).filter(isPublicProduct);
}
...
export async function getPublicCatalogProductBySlug(slug: string) {
  ...
  return product && isPublicProduct(product) ? product : undefined;
}
...
export async function getPublicCatalogProductsByCategory(categoryId: string) {
  const publicProducts = (await getAllProducts()).filter(isPublicProduct);
