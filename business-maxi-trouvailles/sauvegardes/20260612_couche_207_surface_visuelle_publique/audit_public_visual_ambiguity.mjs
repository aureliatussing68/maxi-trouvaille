...
function productCardAirbagFindings() {
  ...
  const checks = [
    {
      ok: source.includes("isPublicProduct(product)"),
      type: "product_card_public_gate_missing",
      reason: "ProductCard doit verifier isPublicProduct avant rendu client.",
    },
    {
      ok: source.includes("const canRenderPublicProduct = isPublicProduct(product);"),
...
function productDetailImageGuardFindings() {
  ...
  const checks = [
    ...
    {
      ok: source.includes("Prévisualisation HOLD"),
      type: "product_detail_admin_hold_warning_missing",
      reason:
        "La preview admin d'une fiche HOLD doit signaler que l'image et l'achat restent bloques.",
    },
    {
