...
function isObviousCodeOnlyLine(line) {
  ...
  return (
    ...
    /\b(isDropshipping|isClientDropshipping|productIds|partnerProducts|highlightedProducts|shopRouteFiles|category\.parentId)\b/.test(trimmed) ||
    /^[}"'),;]+$/.test(trimmed)
  );
}
...
function isLikelyRenderedCopy(line) {
  ...
  return (
    /\b(title|description|label|text|badge|emptyTitle|ctaLabel|titleWhenReady|name|value)\s*:/.test(trimmed) ||
    trimmed.includes(">") ||
    /^(["'`])[^"'`]+(["'`]),?$/.test(trimmed)
  );
}
...
function auditShopRoutes() {
  return shopRouteFiles.flatMap((route) => {
...
function toMarkdown(summary) {
  ...
  return `${[
    ...
    "## Synthese",
    "",
    `- Routes boutique surveillees: ${summary.checkedRouteCount}`,
    ...
    "- La boutique doit rester produits partenaires, paiement Maxi Trouvaille, suivi colis, service client et validation avant vente.",
    "- Les entrees publiques ne doivent exposer aucune source externe ni jargon interne.",
    "",
  ].join("\n")}\n`;
}

const findings = [...auditSupportComponent(), ...auditShopRoutes()];
const summary = {
  checkedAt: new Date().toISOString(),
  ok: findings.length === 0,
  supportComponentPath,
  checkedRouteCount: shopRouteFiles.length,
...
fs.writeFileSync(
  path.join(outputDir, "audit-partner-shop-surface.json"),
  `${JSON.stringify(summary, null, 2)}\n`,
  "utf8",
);
fs.writeFileSync(
  path.join(outputDir, "RAPPORT_AUDIT_PARTNER_SHOP_SURFACE.md"),
  toMarkdown(summary),
  "utf8",
);
