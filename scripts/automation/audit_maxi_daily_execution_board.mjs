...
if (!boardDir || !boardJsonPath) {
  ...
  failures.push(
    ...
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesRunwayStatus ===
        "HOLD_NEXT_WAVE_PENDING_BATCHES_RUNWAY_READY",
      "integration_next_wave_pending_batches_runway_status_invalid",
      "Le tableau n'indique pas une runway lots suivants prete en HOLD.",
      { value: board.metrics?.integrationNextWavePendingBatchesRunwayStatus },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesRunwayAuditStatus ===
        "OK_NEXT_WAVE_PENDING_BATCHES_RUNWAY_GUARDED",
      "integration_next_wave_pending_batches_runway_audit_status_invalid",
      "Le tableau n'indique pas un audit runway lots suivants OK.",
      { value: board.metrics?.integrationNextWavePendingBatchesRunwayAuditStatus },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesRunwayPendingBatchCount === 2 &&
        board.metrics?.integrationNextWavePendingBatchesRunwayPendingProductCount === 8 &&
        board.metrics?.integrationNextWavePendingBatchesRunwayProofTaskCount === 40 &&
        board.metrics?.integrationNextWavePendingBatchesRunwayImageTaskCount === 24 &&
        board.metrics?.integrationNextWavePendingBatchesRunwayTotalTaskCount === 64,
      "integration_next_wave_pending_batches_runway_scope_invalid",
      "La runway lots suivants doit couvrir 2 lots, 8 produits et 64 taches.",
      {
        batchCount: board.metrics?.integrationNextWavePendingBatchesRunwayPendingBatchCount,
        productCount: board.metrics?.integrationNextWavePendingBatchesRunwayPendingProductCount,
        proofTaskCount: board.metrics?.integrationNextWavePendingBatchesRunwayProofTaskCount,
        imageTaskCount: board.metrics?.integrationNextWavePendingBatchesRunwayImageTaskCount,
        totalTaskCount: board.metrics?.integrationNextWavePendingBatchesRunwayTotalTaskCount,
      },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesRunwayActiveBatchBlockedHold === true,
      "integration_next_wave_pending_batches_runway_active_batch_not_blocked",
      "La runway lots suivants doit garder le lot actif bloque HOLD.",
      { value: board.metrics?.integrationNextWavePendingBatchesRunwayActiveBatchBlockedHold },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesRunwayAuditFailureCount === 0,
      "integration_next_wave_pending_batches_runway_audit_failures_not_zero",
      "Le tableau indique des echecs dans l'audit runway lots suivants.",
      { value: board.metrics?.integrationNextWavePendingBatchesRunwayAuditFailureCount },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesRunwayAuditSensitiveFindingCount === 0,
      "integration_next_wave_pending_batches_runway_audit_sensitive_findings_not_zero",
      "Le tableau indique une fuite sensible dans l'audit runway lots suivants.",
      { value: board.metrics?.integrationNextWavePendingBatchesRunwayAuditSensitiveFindingCount },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesFieldEntryPackStatus ===
        "HOLD_NEXT_WAVE_PENDING_BATCHES_FIELD_ENTRY_PACK_READY",
      "integration_next_wave_pending_batches_field_entry_pack_status_invalid",
      "Le tableau n'indique pas un pack saisie terrain lots suivants pret en HOLD.",
      { value: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackStatus },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesFieldEntryPackAuditStatus ===
        "OK_NEXT_WAVE_PENDING_BATCHES_FIELD_ENTRY_PACK_GUARDED",
      "integration_next_wave_pending_batches_field_entry_pack_audit_status_invalid",
      "Le tableau n'indique pas un audit pack saisie terrain lots suivants OK.",
      { value: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackAuditStatus },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesFieldEntryPackPendingBatchCount === 2 &&
        board.metrics?.integrationNextWavePendingBatchesFieldEntryPackProductCount === 8 &&
        board.metrics?.integrationNextWavePendingBatchesFieldEntryPackEntryCount === 64 &&
        board.metrics?.integrationNextWavePendingBatchesFieldEntryPackProofEntryCount === 40 &&
        board.metrics?.integrationNextWavePendingBatchesFieldEntryPackWebpEntryCount === 24 &&
        board.metrics?.integrationNextWavePendingBatchesFieldEntryPackProductSheetCount === 8,
      "integration_next_wave_pending_batches_field_entry_pack_scope_invalid",
      "Le pack saisie terrain lots suivants doit couvrir 2 lots, 8 produits, 40 preuves, 24 WebP et 8 fiches.",
      {
        batchCount: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackPendingBatchCount,
        productCount: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackProductCount,
        entryCount: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackEntryCount,
        proofEntryCount: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackProofEntryCount,
        webpEntryCount: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackWebpEntryCount,
        sheetCount: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackProductSheetCount,
      },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesFieldEntryPackBlockedEntryCount === 64 &&
        board.metrics?.integrationNextWavePendingBatchesFieldEntryPackReadyEntryCount === 0,
      "integration_next_wave_pending_batches_field_entry_pack_readiness_invalid",
      "Toutes les entrees du pack lots suivants doivent rester bloquees HOLD.",
      {
        blocked: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackBlockedEntryCount,
        ready: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackReadyEntryCount,
      },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesFieldEntryPackAuditFailureCount === 0,
      "integration_next_wave_pending_batches_field_entry_pack_audit_failures_not_zero",
      "Le tableau indique des echecs dans l'audit pack saisie terrain lots suivants.",
      { value: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackAuditFailureCount },
    ),
    assertCondition(
      board.metrics?.integrationNextWavePendingBatchesFieldEntryPackAuditSensitiveFindingCount === 0,
      "integration_next_wave_pending_batches_field_entry_pack_audit_sensitive_findings_not_zero",
      "Le tableau indique une fuite sensible dans l'audit pack saisie terrain lots suivants.",
      { value: board.metrics?.integrationNextWavePendingBatchesFieldEntryPackAuditSensitiveFindingCount },
    ),
    assertCondition(
