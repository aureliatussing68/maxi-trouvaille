...
function manifestImage(product, gateProduct, targetImage) {
  const currentSupplierImage = gateProduct.supplierDomainImages[targetImage.index - 1] ?? null;

  return {
    index: targetImage.index,
    role: targetImage.role,
    currentSupplierUrl: currentSupplierImage ? REMOTE_IMAGE_SOURCE_REDACTED : "",
    currentSupplierHost: currentSupplierImage ? REMOTE_IMAGE_HOST_REDACTED : "",
