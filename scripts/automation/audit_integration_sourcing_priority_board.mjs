...
function validateBoardArtifacts({ boardPath, board, session, sessionAudit, files }) {
  ...
  for (const row of board.rows ?? []) {
    ...
    if (expectedImageFiles.length !== (Number(row.imageTaskCount) || 0)) {
      addIssue(issues, "rows", "EXPECTED_IMAGE_FILE_COUNT_MISMATCH", "La liste des fichiers WebP attendus est incoherente.", {
        productId: row.productId,
        expectedImageFileCount: expectedImageFiles.length,
        imageTaskCount: row.imageTaskCount,
      });
    }

    expectedImageFiles.forEach((fileName, index) => {
