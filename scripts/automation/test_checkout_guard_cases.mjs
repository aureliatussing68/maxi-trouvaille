...
const publicPurchasableCount = eligibilitySummary?.expectedPurchasableCount ?? 0;
...
const cases = [
  ...
  caseResult({
    ...
    passed:
      sourceGuards.apiRejectsOverStockQuantity &&
      (Boolean(purchasableProduct) || publicPurchasableCount === 0),
    evidence:
      publicPurchasableCount === 0
        ? "No public purchasable product while dropshipping stays HOLD; route still compares requested quantity to product.stock."
        : "Route compares requested quantity to product.stock.",
  }),
  caseResult({
  ...
  caseResult({
    ...
    evidence: "Route requires STRIPE_ENABLE_LIVE_PAYMENTS before live payment mode.",
  }),
];
