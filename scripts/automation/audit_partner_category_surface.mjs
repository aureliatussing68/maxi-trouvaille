...
function isObviousCodeOnlyLine(line) {
  ...
  return (
    ...
    /\b(categorySlug|kind|matchesProduct|isDropshipping|productIds|partnerProducts|legacyCategoryRedirects|item\.parentId)\b/.test(trimmed) ||
    /^[}"'),;]+$/.test(trimmed)
  );
}
...
function auditSupportComponent() {
  ...
  const linkFindings = requiredSupportLinks.flatMap((href) =>
  ...
  return [
    ...linkFindings,
    ...missingSignalFindings(
      supportComponentPath,
      source,
      requiredSupportSignals,
    ),
    ...scanForbidden(supportComponentPath, source),
  ];
}

function auditCategoryRoutes() {
  return categoryRouteFiles.flatMap((route) => {
...
function toMarkdown(summary) {
  ...
  return `${[
    "# Audit surface rayons partenaires",
    "",
    ...
    "## Synthese",
    "",
    `- Routes rayons surveillees: ${summary.checkedRouteCount}`,
    ...
    "- Les rayons publics doivent garder paiement Maxi Trouvaille, suivi colis, service client et validation avant vente.",
    "- Les liens support doivent rester accessibles depuis la navigation mobile des rayons.",
    "",
  ].join("\n")}\n`;
}

const findings = [...auditSupportComponent(), ...auditCategoryRoutes()];
const summary = {
  checkedAt: new Date().toISOString(),
  ok: findings.length === 0,
  supportComponentPath,
  checkedRouteCount: categoryRouteFiles.length,
...
fs.writeFileSync(
  path.join(outputDir, "audit-partner-category-surface.json"),
  `${JSON.stringify(summary, null, 2)}\n`,
  "utf8",
);
fs.writeFileSync(
  path.join(outputDir, "RAPPORT_AUDIT_PARTNER_CATEGORY_SURFACE.md"),
  toMarkdown(summary),
  "utf8",
);
