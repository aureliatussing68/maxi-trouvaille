...
const catalogClientPath = path.join(root, "src", "lib", "catalog-client.ts");
const cartProviderPath = path.join(root, "src", "components", "CartProvider.tsx");
const cartViewPath = path.join(root, "src", "components", "CartView.tsx");
...
const cartEligibilityRoutePath = path.join(root, "src", "app", "api", "cart", "eligible-items", "route.ts");
const cartPagePath = path.join(root, "src", "app", "panier", "page.tsx");
const paymentPagePath = path.join(root, "src", "app", "paiement", "page.tsx");
const checkoutRoutePath = path.join(root, "src", "app", "api", "checkout", "route.ts");
const adminProductsRoutePath = path.join(root, "src", "app", "api", "admin", "products", "route.ts");
const adminProductsDecrementRoutePath = path.join(root, "src", "app", "api", "admin", "products", "decrement", "route.ts");
const adminProductsPhotoAnalysisRoutePath = path.join(root, "src", "app", "api", "admin", "products", "photo-analysis", "route.ts");
...
function sourceGuards() {
  ...
  const catalogClientSource = readFile(catalogClientPath);
  const cartProviderSource = readFile(cartProviderPath);
  const cartViewSource = readFile(cartViewPath);
  const checkoutViewSource = readFile(checkoutViewPath);
  const orderSuccessSource = readFile(orderSuccessPath);
  const cartEligibilityRouteSource = readFile(cartEligibilityRoutePath);
  const cartPageSource = readFile(cartPagePath);
  const paymentPageSource = readFile(paymentPagePath);
  const checkoutRouteSource = readFile(checkoutRoutePath);
  const adminProductsRouteSource = readFile(adminProductsRoutePath);
  const adminProductsDecrementRouteSource = readFile(adminProductsDecrementRoutePath);
  const adminProductsPhotoAnalysisRouteSource = readFile(adminProductsPhotoAnalysisRoutePath);
  const purchasableBody = extractFunctionBody(catalogSource, "isProductPurchasable");
  const normalizedCartEligibilityRoute = normalizeText(cartEligibilityRouteSource);
  ...
  const publicProductHasReadinessGate =
    ...
    catalogSource.includes("getPublicImageBlockers(product)");
  ...
  return {
    ...
    catalogClientDoesNotImportCatalogValues:
      /import type[\s\S]+from ["']@\/lib\/catalog["']/.test(catalogClientSource) &&
      !/import\s+(?!type)[\s\S]+from ["']@\/lib\/catalog["']/.test(catalogClientSource),
    cartProviderStoresOnlyCartLines:
      !/from ["']@\/lib\/catalog["']/.test(cartProviderSource) &&
      !/\bproducts\b|getProductById|staticProductMap|detailedItems|subtotal|lineTotal/.test(
        cartProviderSource,
      ),
    cartProviderFetchesPublicEligibility:
      /fetch\(["']\/api\/cart\/eligible-items["']/.test(cartProviderSource) &&
      /cache: ["']no-store["']/.test(cartProviderSource),
    cartProviderPurgesUnavailableStoredItems:
      /reconcileCartItems/.test(cartProviderSource) &&
      /currentItems\.flatMap/.test(cartProviderSource) &&
      /if \(!eligibleStock\)/.test(cartProviderSource) &&
      /return \[\]/.test(cartProviderSource),
    cartProviderClampsEligibleQuantities:
      /clampQuantity\(item\.quantity, eligibleStock\)/.test(cartProviderSource),
    cartProviderAddItemStoresOnlyIds:
      /const addItem = useCallback\(\(productId: string/.test(cartProviderSource) &&
      /return \[\.\.\.currentItems, \{ productId, quantity: cappedQuantity \}\]/.test(
        cartProviderSource,
      ),
    cartProviderUpdateQuantityDoesNotReadCatalog:
      /const updateQuantity = useCallback\(\(productId: string, quantity: number\)/.test(
        cartProviderSource,
      ) && !/updateQuantity[\s\S]+isProductPurchasable/.test(cartProviderSource),
    cartProviderDoesNotFetchAdminProducts:
      !/fetch\(["']\/api\/admin\/products["']/.test(cartProviderSource),
    cartProviderDoesNotHydrateQuickDrafts:
      !/sanitizeQuickProducts|QUICK_PRODUCTS_UPDATED_EVENT|quickProductsLoaded/.test(
        cartProviderSource,
      ),
    adminProductsApiRequiresAdminMode:
      /isAdminModeEnabled\(\)/.test(adminProductsRouteSource) &&
      /export async function GET\(\)[\s\S]+if \(!isAdminModeEnabled\(\)\)/.test(
        adminProductsRouteSource,
      ) &&
      /export async function POST\(request: Request\)[\s\S]+if \(!isAdminModeEnabled\(\)\)/.test(
        adminProductsRouteSource,
      ),
    orderSuccessDoesNotCallAdminStockDecrement:
      !/\/api\/admin\/products\/decrement/.test(orderSuccessSource),
    adminProductsDecrementRequiresAdminMode:
      /isAdminModeEnabled\(\)/.test(adminProductsDecrementRouteSource) &&
      /export async function POST\(request: Request\)\s*{\s*if \(!isAdminModeEnabled\(\)\)/.test(
        adminProductsDecrementRouteSource,
      ),
    adminProductsPhotoAnalysisRequiresAdminMode:
      /isAdminModeEnabled\(\)/.test(adminProductsPhotoAnalysisRouteSource) &&
      /export async function POST\(request: Request\)\s*{\s*if \(!isAdminModeEnabled\(\)\)/.test(
        adminProductsPhotoAnalysisRouteSource,
      ),
    cartProviderClampsStoredQuantities:
      /quantity: clampQuantity\(quantity\)/.test(
        cartProviderSource,
      ),
    ...
    cartEligibilityApiDoesNotExposeSensitiveFields:
      !normalizedCartEligibilityRoute.includes("supplier") &&
      !normalizedCartEligibilityRoute.includes("aliexpress") &&
      !normalizedCartEligibilityRoute.includes("image") &&
      !normalizedCartEligibilityRoute.includes("price") &&
      !normalizedCartEligibilityRoute.includes("margin"),
    cartPageProvidesPublicProducts:
      /getPublicProducts/.test(cartPageSource) &&
      /<CartView products=\{products\}/.test(cartPageSource) &&
      /dynamic = ["']force-dynamic["']/.test(cartPageSource),
    paymentPageProvidesPublicProducts:
      /getPublicProducts/.test(paymentPageSource) &&
      /<CheckoutView products=\{products\}/.test(paymentPageSource) &&
      /dynamic = ["']force-dynamic["']/.test(paymentPageSource),
    cartViewUsesPublicProductsProp:
      /export function CartView\(\{ products \}: \{ products: Product\[\] \}\)/.test(
        cartViewSource,
      ) && /buildDetailedCartItems\(items, products\)/.test(cartViewSource),
    cartViewDetailedItemsHideUnavailableProducts:
      /if \(!product \|\| !isClientProductPurchasable\(product\)\)/.test(
        cartViewSource,
      ) && /lineTotal: product\.price \* quantity/.test(cartViewSource),
    checkoutViewUsesPublicProductsProp:
      /export function CheckoutView\(\{ products \}: \{ products: Product\[\] \}\)/.test(
        checkoutViewSource,
      ) && /buildDetailedCartItems\(items, products\)/.test(checkoutViewSource),
    checkoutViewBlocksUnavailableItems:
      /blockedItems[\s\S]+!isClientProductPurchasable\(item\.product\)/.test(
        checkoutViewSource,
      ) &&
