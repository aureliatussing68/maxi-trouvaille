...
function imageInfo(rawUrl, index) {
  ...
  const info = {
    index,
    url: value && !value.startsWith("/") ? REMOTE_IMAGE_SOURCE_REDACTED : value,
    sourceType: "unknown",
    hostname: "",
    isSupplierCdn: false,
    isLocalUpload: false,
  ...
  try {
    ...
    return {
      ...
      hostname: REMOTE_IMAGE_HOST_REDACTED,
      isSupplierCdn,
      clientSafeIfPublished: false,
    };
  } catch {
...
function toRecord(candidate, product) {
  ...
  return {
    ...
    supplierReference: product?.dropshipping?.supplierUrl || candidate.supplier?.url ? "INTERNAL_SOURCE_REDACTED" : "",
