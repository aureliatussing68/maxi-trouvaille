...
const checkoutRouteFiles = [
  ...
  {
    path: "src/components/CartView.tsx",
    requiredSignals: [
  ...
  {
    path: "src/components/CheckoutView.tsx",
    requiredSignals: [
      "isClientProductPurchasable",
      "/api/checkout",
      "/produits-partenaires",
...
function isObviousCodeOnlyLine(line) {
  ...
  return (
    ...
    /\b(isDropshipping|isClientProductPurchasable|checkoutRouteFiles|requiredSignals|blockedItems|validation|response|data|fetch)\b/.test(trimmed) ||
    /^[}"'),;]+$/.test(trimmed)
  );
}
...
function auditCheckoutRoutes() {
  return checkoutRouteFiles.flatMap((route) => {
...
function toMarkdown(summary) {
  ...
  return `${[
    ...
    "## Synthese",
    "",
    `- Fichiers parcours surveilles: ${summary.checkedFileCount}`,
    ...
    "- Panier et paiement doivent garder le vocabulaire Maxi Trouvaille, suivi colis, service client et validation avant vente.",
    "- Le parcours client ne doit exposer aucune source externe ni jargon interne.",
    "",
  ].join("\n")}\n`;
}

const findings = [...auditSupportComponent(), ...auditCheckoutRoutes()];
const summary = {
  checkedAt: new Date().toISOString(),
  ok: findings.length === 0,
  supportComponentPath,
  checkedFileCount: checkoutRouteFiles.length,
...
fs.writeFileSync(
  path.join(outputDir, "audit-partner-checkout-surface.json"),
  `${JSON.stringify(summary, null, 2)}\n`,
  "utf8",
);
fs.writeFileSync(
  path.join(outputDir, "RAPPORT_AUDIT_PARTNER_CHECKOUT_SURFACE.md"),
  toMarkdown(summary),
  "utf8",
);
