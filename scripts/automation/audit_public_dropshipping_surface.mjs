...
const publicRootPath = path.join(root, "public");
...
const catalogServerSource = readFile(catalogServerPath);
...
const productCardGuardFailures = [
  {
    ok:
      productCardSource.includes("@/lib/catalog-client") &&
      !/from\s+["']@\/lib\/catalog["']/.test(productCardSource),
    reason: "product_card_client_safe_catalog_helpers_missing",
  },
  {
...
const catalogServerGuardFailures = [
  {
    ok: catalogServerSource.includes("getPublicImageFileBlockers(product)"),
    reason: "catalog_server_image_file_gate_missing",
  },
  {
    ok: catalogServerSource.includes("fs.stat(localPath)"),
    reason: "catalog_server_image_file_existence_check_missing",
  },
  {
    ok: catalogServerSource.includes("getPublicProducts()") &&
      catalogServerSource.includes("filterServerPublicProducts"),
    reason: "catalog_server_public_filter_missing",
  },
].flatMap((check) =>
  check.ok
    ? []
    : [
        {
          scope: "catalog_server_source",
          file: "src/lib/catalog-server.ts",
          reason: check.reason,
        },
      ],
);
