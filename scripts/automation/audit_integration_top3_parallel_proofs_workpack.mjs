...
const workpackRoot = path.join(
  ...
  "tableaux-action",
  "top3-preuves-paralleles-sourcing-integration-articles",
);
const outputRoot = path.join(
  root,
  "business-maxi-trouvailles",
  "tableaux-action",
  "audit-top3-preuves-paralleles-sourcing-integration-articles",
);
...
function validateWorkpack({ workpack, csv, fillableCsvPath }) {
  const issues = [];
  const proofs = workpack.proofs ?? [];
  const expectedProofCount = (workpack.productCount ?? 0) * (workpack.proofsPerProductTarget ?? 0);
  const proofKeys = new Set(proofs.map(proofKey));
  const csvKeys = new Set(csv.rows.map(csvProofKey));

  if (workpack.mode !== "read_only_integration_top3_parallel_proofs_workpack") {
    addIssue(issues, "workpack", "invalid_mode", "Le mode du workpack parallele est inattendu.", {
      mode: workpack.mode,
    });
  }

  if (workpack.status !== "HOLD_TOP3_PARALLEL_PROOFS_WORKPACK_READY") {
    addIssue(issues, "workpack", "invalid_status", "Le workpack parallele doit rester pret en HOLD.", {
  ...
  if (workpack.productCount !== 3 || workpack.proofCount !== expectedProofCount || proofs.length !== expectedProofCount) {
    addIssue(issues, "counts", "proof_count_mismatch", "Le volume de preuves paralleles est incoherent.", {
      productCount: workpack.productCount,
      proofCount: workpack.proofCount,
  ...
  if (!fs.existsSync(fillableCsvPath)) {
    addIssue(issues, "csv", "fillable_csv_missing", "Le CSV remplissable du workpack est introuvable.", {
      file: rel(fillableCsvPath),
    });
  }

  if (csv.rows.length !== proofs.length) {
    addIssue(issues, "csv", "csv_row_count_mismatch", "Le CSV a remplir ne suit pas les preuves JSON.", {
  ...
  for (const proof of proofs) {
    if (!criticalFieldKeys.has(proof.fieldKey)) {
      addIssue(issues, "proof", "non_critical_field", "Une preuve n'appartient pas au set critique.", {
    ...
    if (!isInternalAdminHref(proof.adminHref)) {
      addIssue(issues, "proof", "admin_href_not_internal", "Lien admin preuve non interne.", {
        productId: proof.productId,
        fieldKey: proof.fieldKey,
      });
    }

    if (proof.manualInput?.finalDecision !== "HOLD_TO_FILL") {
      addIssue(issues, "proof", "manual_decision_not_hold", "La decision manuelle doit rester HOLD_TO_FILL.", {
        productId: proof.productId,
        fieldKey: proof.fieldKey,
      });
    }

    if (!Array.isArray(proof.expectedImageFiles) || proof.expectedImageFiles.length !== 3) {
      addIssue(issues, "proof", "expected_images_invalid", "Chaque preuve top 3 doit rappeler 3 WebP attendus.", {
        productId: proof.productId,
        fieldKey: proof.fieldKey,
        expectedImageCount: proof.expectedImageFiles?.length ?? 0,
      });
    }

    if (!csvKeys.has(proofKey(proof))) {
      addIssue(issues, "csv", "proof_missing_from_csv", "Une preuve JSON manque dans le CSV remplissable.", {
        key: proofKey(proof),
      });
    }
  }
  ...
  for (const row of csv.rows) {
    if (!proofKeys.has(csvProofKey(row))) {
      addIssue(issues, "csv", "unexpected_csv_row", "Le CSV contient une preuve absente du JSON.", {
        key: csvProofKey(row),
      });
    }
  }
...
function markdown(summary) {
  ...
  return `${[
    "# Audit top 3 preuves paralleles sourcing",
    "",
    ...
    `- Preuves critiques: ${summary.proofCount}`,
...
const workpackPath = latestFile(workpackRoot, /TOP3_PREUVES_PARALLELES_SOURCING_INTEGRATION_\d+\.json$/);
...
const fillableCsvPath = path.join(workpackDir, `A_REMPLIR_TOP3_PREUVES_PARALLELES_SOURCING_${workpackKey}.csv`);
const csv = readCsv(fillableCsvPath);
const workpackFiles = [
  workpackPath,
  path.join(workpackDir, `TOP3_PREUVES_PARALLELES_SOURCING_INTEGRATION_${workpackKey}.md`),
  path.join(workpackDir, `top3-preuves-paralleles-sourcing-integration-${workpackKey}.csv`),
  fillableCsvPath,
  ...walkFiles(path.join(workpackDir, "preuves"), (filePath) => [".json", ".md"].includes(path.extname(filePath))),
...
const summary = {
  ...
  mode: "read_only_integration_top3_parallel_proofs_workpack_audit",
  status:
    issues.length === 0 && sensitiveFindings.length === 0
      ? "OK_TOP3_PARALLEL_PROOFS_GUARDED"
      : "FAIL_TOP3_PARALLEL_PROOFS_GUARDS",
  productCount: workpack.productCount ?? 0,
  proofCount: workpack.proofCount ?? 0,
  ...
  sources: {
    ...
    sessionPath: workpack.sources?.sessionPath ?? "",
  },
  safety: {
    readOnlyAudit: true,
    noCatalogWrite: true,
...
const jsonPath = path.join(outputDir, `AUDIT_TOP3_PREUVES_PARALLELES_SOURCING_INTEGRATION_${dateKey}.json`);
const mdPath = path.join(outputDir, `AUDIT_TOP3_PREUVES_PARALLELES_SOURCING_INTEGRATION_${dateKey}.md`);
const csvPath = path.join(outputDir, `audit-top3-preuves-paralleles-sourcing-integration-${dateKey}.csv`);
