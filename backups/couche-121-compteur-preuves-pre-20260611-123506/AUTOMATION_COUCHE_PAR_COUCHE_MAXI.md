...
La page admin `Preuves partenaires` permet de rechercher les fiches HOLD par nom, slug, categorie ou statut, puis d'exporter le resultat filtre en CSV terrain sans modifier le catalogue. Elle affiche aussi un `Lot terrain du jour` avec les 3 premieres fiches HOLD a verrouiller, chacune avec etat visuel priorise (`Image a prouver`, `Marge a verrouiller` ou `Delai a prouver`), prochaine action terrain, signaux image/marge/livraison, checklist compacte, lien `Fiche terrain` filtre en `status=hold` et export CSV dedie au lot du jour. Ce CSV conserve `priorite_visuelle`, `etat_visuel` et `action_terrain` pour traiter le lot dans le bon ordre hors interface. Le `Top produits a verifier maintenant` permet ensuite d'attaquer en premier les fiches avec signaux image, marge, livraison et potentiel categorie, avec un export CSV court dedie au top prioritaire, une mini fiche terrain par produit et un mode impression limite au top verification.

La page admin `Pilotage` affiche un recap `HOLD du jour`: nombre de fiches partenaires a prouver, top verification pret, CSV court pret, impression prete et prochain produit a verifier. Le bouton `Fiche terrain` ouvre la page preuves avec `status=hold`, un filtre `q` sur le produit cible et une ancre de carte top verification. Elle reste une vue de pilotage, sans publication, paiement ni commande fournisseur.
