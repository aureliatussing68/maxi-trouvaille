...
function auditProducts(products) {
  return products.map((product) => {
    ...
    if (product.dropshippingEnabled && dropshippingFocusCategoryIds.has(product.categoryId)) {
      if (!product.supplierUrl) publicationBlockers.push("supplier_url_missing");
      if (!product.supplierSku) publicationBlockers.push("supplier_sku_missing");
      if (!product.supplierPriceCents) publicationBlockers.push("supplier_price_missing");
      if (!product.deliveryEstimate) publicationBlockers.push("delivery_estimate_missing");
      if (imageRiskReasons(product).length > 0) publicationBlockers.push("image_not_public_ready");
      if (/hold/i.test(product.validationGateNote)) publicationBlockers.push("validation_gate_hold");
    }
...
const failures = [...productFailures, ...sourceFailures];
