...
export default async function AdminPartnerProofsPage({
  ...
  const hasProofFilter = Boolean(normalizedProofQuery || proofStatus !== "all");
  ...
  const filteredNextActions = nextActions.filter((action) =>
    matchesProofSearch(normalizedProofQuery, [
      action.id,
      action.name,
      action.lane,
      action.status,
  ...
  const filteredFastProofNowProducts = fastProofNowProducts.filter((product) =>
    matchesProofSearch(normalizedProofQuery, [
      ...
      product.categoryId,
      product.status,
      product.missingFields.map((field) => field.label).join(" "),
    ]) &&
  ...
  const proofExportFilename = `maxi-preuves-partenaires-${proofStatus}-${csvFilenamePart(
    proofQuery,
  )}.csv`;
  ...
  const topVerificationExportFilename = `maxi-top-verification-${proofStatus}-${csvFilenamePart(
    proofQuery,
  )}.csv`;
  ...
  const terrainLotItems = topVerificationItems.filter(isHoldTerrainItem).slice(0, 3);
  const terrainLotExportFilename = `maxi-lot-terrain-du-jour-${proofStatus}-${csvFilenamePart(
    proofQuery,
  )}.csv`;
