...
export const productConditionOptions = [
  ...
  { value: "neuf_sans_emballage", label: "Neuf sans emballage" },
  { value: "tres_bon_etat", label: "Très bon état" },
  { value: "destockage", label: "Déstockage" },
  { value: "colis_perdu", label: "Retour / déstockage" },
...
export function normalizeProductCondition(value: unknown): ProductConditionValue {
  ...
  if (normalized.includes("neuf sans emballage")) {
    return "neuf_sans_emballage";
  }

  if (normalized.includes("tres bon") || normalized.includes("quasi neuf")) {
    return "tres_bon_etat";
  }

  if (normalized.includes("destockage") || normalized.includes("fin de serie")) {
    return "destockage";
  }

  if (normalized.includes("colis perdu") || normalized.includes("retour client")) {
    return "colis_perdu";
  }

  if (normalized.includes("a verifier") || normalized.includes("verifier")) {
    return "a_verifier";
  }
  ...
  return productConditionOptions.some((condition) => condition.value === value)
    ? (value as ProductConditionValue)
    : "a_verifier";
}

export function getProductConditionLabel(value: unknown) {
  const condition = normalizeProductCondition(value);
  return (
    productConditionOptions.find((option) => option.value === condition)?.label ??
    "À vérifier"
  );
}

export function normalizeKeywords(value: unknown) {
...
function getCategoryTone(categoryId: string) {
  const tones: Record<string, { usage: string; benefit: string }> = {
    "colis-surprise-palettes": {
      usage:
        "adapte aux palettes, colis mystères, colis au poids et lots surprise issus de déstockage",
      benefit: "un rayon clair pour les arrivages surprise et les bonnes affaires",
    },
    "palettes-destockage": {
      usage:
...
export function generateCommerceDescription(input: DescriptionGenerationInput) {
  ...
  const normalizedCondition = normalizeProductCondition(input.condition);
  const conditionDisplay = getProductConditionLabel(input.condition);
  const conditionLabel =
    normalizedCondition === "a_verifier"
      ? "état à vérifier"
      : conditionDisplay.toLowerCase();
  const introCondition =
    normalizedCondition === "a_verifier"
      ? "dont l'état reste à vérifier"
      : conditionLabel;
  ...
  const strengths = normalizeTextList(input.strengths, 5);
  const marketPriceText = input.marketPriceEstimate?.trim()
    ? ` Prix neuf estime : ${input.marketPriceEstimate.trim()}.`
    : "";
  const priceReviewText = input.priceReviewNote?.trim()
    ? ` ${input.priceReviewNote.trim()}.`
    : "";
  const keywordText = keywords.length > 0 ? `\nMots-cles : ${keywords.join(", ")}` : "";

  return [
    `${title}, ${introCondition}, selectionne pour Maxi Trouvaille. Ce produit est ${tone.usage}.${priceText}${marketPriceText}${priceReviewText}${imageText}`,
    "",
    "Points forts :",
    ...(strengths.length > 0
      ? strengths.map((strength) => `- ${strength}`)
      : [`- ${tone.benefit}`]),
    `- Etat : ${conditionDisplay}`,
    `- Categorie : ${category?.name ?? "Nouveautés"}`,
...
export function detectCategoryId(text: string) {
  ...
  return detected?.categoryId ?? "dropshipping-nouveautes";
}
...
export function createQuickProduct(input: QuickProductInput): Product {
  ...
  const keywords = normalizeKeywords(input.keywords);
  const strengths = normalizeTextList(input.strengths, 6);
  const marketPriceEstimate = input.marketPriceEstimate?.trim();
  const priceReviewNote = input.priceReviewNote?.trim();
  const productType = input.productType?.trim();
  const usage = input.usage?.trim();
  const categoryId = getCategoryById(input.categoryId)
    ? input.categoryId
    : detectCategoryId(
        `${cleanTitle} ${cleanDescription} ${keywords.join(" ")} ${productType ?? ""}`,
      );
  const price = parsePriceToCents(input.price);
  const stock = Math.max(0, Math.trunc(Number(input.stock ?? 1)) || 0);
  const conditionLabel = getProductConditionLabel(input.condition);
  ...
  return {
    ...
    stock,
    badge: stock > 0 ? "HOLD verification" : "Rupture",
    image: mainImage,
    ...
    features: [
      ...
      ...(marketPriceEstimate ? [`Prix neuf estime : ${marketPriceEstimate}`] : []),
      ...(priceReviewNote ? [priceReviewNote] : []),
      ...(keywords.length > 0 ? [`Mots-cles : ${keywords.join(", ")}`] : []),
    ],
    source: "internal",
    status: "draft",
...
export function sanitizeQuickProducts(input: unknown): Product[] {
  ...
  return input.flatMap((product): Product[] => {
    ...
    const status =
      maybeProduct.status === "published" || maybeProduct.status === "archived"
        ? maybeProduct.status
        : "draft";
