...
La page admin `Preuves partenaires` permet de rechercher les fiches HOLD par nom, slug, categorie ou statut, puis d'exporter le resultat filtre en CSV terrain sans modifier le catalogue. Elle affiche aussi un `Top produits a verifier maintenant` pour attaquer en premier les fiches avec signaux image, marge, livraison et potentiel categorie, avec un export CSV court dedie au top prioritaire, une mini fiche terrain par produit et un mode impression limite au top verification.
