...
function sourceGuards() {
  ...
  return {
    ...
    cartProviderPurgesUnavailableStoredItems:
      /setItems\(\(currentItems\) =>[\s\S]+currentItems\.flatMap/.test(
        cartProviderSource,
      ) &&
      /!product \|\| !isProductPurchasable\(product\)/.test(cartProviderSource),
    cartProviderClampsStoredQuantities:
      /const quantity = clampQuantity\(item\.quantity, product\.stock\)/.test(
        cartProviderSource,
      ),
    cartProviderDetailedItemsHideUnavailableProducts:
      /if \(!product \|\| !isProductPurchasable\(product\)\)/.test(
        cartProviderSource,
      ) &&
      /lineTotal: product\.price \* quantity/.test(cartProviderSource),
