...
function cockpitActions(source) {
...
function markdown(summary) {
  ...
  return `${[
    ...
    `- Cockpit produit actif: ${summary.metrics.cockpitEvidenceFieldMissingCount} preuves, ${summary.metrics.cockpitImageTaskCount} images, ${summary.metrics.cockpitBlockerCount} blocages`,
...
const cockpitPath = latestFileUnder(actionRoot, "COCKPIT_VALIDATION_PRODUIT_");
...
const cockpit = readJsonIfExists(cockpitPath);
...
const summary = {
  ...
  metrics: {
    ...
    cockpitEvidenceFieldMissingCount: cockpit?.evidenceFieldMissingCount ?? 0,
    cockpitImageTaskCount: cockpit?.imageTaskCount ?? 0,
    cockpitBlockerCount: cockpit?.blockers?.length ?? 0,
