...
function buildZoneSprintCsv(rows: TopVerificationItem[], zone: string) {
  const headers = [
    "ordre_zone",
    "zone_preuve",
    "score",
    ...
    "blocages",
    "session_image_ok",
    "session_fournisseur_ok",
    "session_prix_ok",
    "session_livraison_ok",
    "session_validation_mouss",
    "note_session",
    "lien_sprint",
  ];
  const lines = rows.map((row, index) => {
    ...
    return [
      ...
      "",
      "",
      zoneSprintItemHref(row, "hold", zone),
    ]
      .map(csvCell)
      .join(",");
  });
...
export default async function AdminPartnerProofsPage({
  ...
  const zoneSprintExportFilename = `maxi-sprint-zone-${proofStatus}-${proofZone}-${csvFilenamePart(
    proofQuery,
  )}.csv`;
  const zoneSprintExportHref = `data:text/csv;charset=utf-8,${encodeURIComponent(
    buildZoneSprintCsv(zoneSprintItems, proofZone),
  )}`;
          ...
          >
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="flex shrink-0 flex-wrap gap-2">
                <span className="inline-flex min-h-10 items-center rounded-md border border-[#f6d38b] bg-white px-3 text-xs font-black uppercase text-[#8a5a00]">
                  {zoneSprintCount} preuves zone
                </span>
                <a
                  href={zoneSprintExportHref}
                  download={zoneSprintExportFilename}
                  className="focus-ring inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-[#f6d38b] bg-white px-3 text-xs font-black uppercase text-[#8a5a00] hover:bg-[#fff3c4]"
                >
              ...
              {zoneSprintItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 rounded-md border border-[#f6d38b] bg-[#fffaf0] p-3">
                      <p className="text-[11px] font-black uppercase text-[#8a5a00]">
                        Checklist session
                      </p>
                      <div className="mt-2 space-y-2">
                        {topVerificationChecklist(item).map((task, taskIndex) => (
                          <label
                            key={`zone-sprint-session-${item.slug}-${taskIndex}`}
                            className="flex min-w-0 items-start gap-2 text-xs font-bold leading-5 text-foreground"
                          >
                            <input
                              type="checkbox"
                              name={`zone-sprint-session-${item.slug}-${taskIndex}`}
                              aria-label={`Session terrain ${item.slug} ${task}`}
                              className="mt-1 h-4 w-4 shrink-0 accent-[#8a5a00]"
                            />
